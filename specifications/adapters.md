...
**协议状态不能越过本地判断：**A2A `completed`、HTTP 2xx、MCP ToolResult、模型停止原因或连接关闭只说明外部协议发生了什么。本地系统仍要分别核对精确身份、动作授权、evidence entry 覆盖、目标满足和最终接受。

> **当前策略：**`ADP-CORE 0.1.0-draft` 只定义协议无关的抽象责任。物理格式不适用，当前尚无适配器、协议 Profile、凭据代理、重试引擎或 bounded runner 实现。
...
| `ADP-CAP-001`<br>能力交集 | 协议、对端、适配器、session contract 和政策的共同能力 | 未知能力关闭失败，协商和错误建议不得扩大当前会话 |
...
| [ADP-CORE 向量](https://github.com/Noemion/noemion.github.io/tree/main/vectors/adapters) | 覆盖允许包络与确定拒绝，并与当前条款保持对应 | MCP、A2A、HTTP、SDK、适配器或 bounded runner 已经实现 |
