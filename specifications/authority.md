...
| 区分主体 | 发布请求者、具体 CI 运行实例，以及它所代表的发布团队 | 只有模型名、Agent 名、服务账户或密钥时，不能恢复实际行动者 |
...
| 派生能力 | 决定范围与 contract、制品上限、环境、适配器和预算的交集 | 任何上限缺失或已经漂移时关闭对应能力；扩大范围必须新建会话 |
...
| 证据是否足够 | evidence 的有效性、覆盖范围和适用判据 | 声明范围内的证据判断 | 授权主体已经最终接受 |
