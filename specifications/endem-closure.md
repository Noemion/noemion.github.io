---
layout: content
title: Endem closure · 组合闭包规范
page_role: content
footer_text: Noemion · Endem closure
...
- label: 规范参考指南
  url: index.html
page_heading: 组合闭包 · Endem closure
...
next_label: session contract 会话契约
---

## 先判断它是不是两个目标

一次依赖升级既要产生使用获准依赖的发布制品，也要让部署后的服务通过健康检查。两者能够分别满足、拒绝和重试，因此各自形成 Endem；组合闭包（设计阶段名称 Endem closure）再把它们和全部必需依赖固定为一个交付范围。
...
| 先问什么 | 使用 Endem closure | 继续使用一个 Endem |
| --- | --- | --- |
...
> **职责边界：**Endem closure 负责组合多个目标，不把文件列表、目录扫描、模型记忆、对话上下文或动态工具清单改称目标闭包。
...
| 问题 | 应当保存什么 | 不能推出什么 |
| --- | --- | --- |
| 一个成员得到结果<br>`CLOSURE-STA-001` | 该 Endem 自己的 `met / unmet / undetermined / fault`、决定和依据 | 另一个成员或整个 Endem closure 自动 `met`、`accepted` 或 `completed` |
...
**复核日期：**2026-07-16。下列机制可以解释某一项工程纪律，但都不能直接成为 Endem closure 成员、权限或结果。
...
| [GNU Guix 递归归档](https://guix.gnu.org/manual/en/guix.pdf) | `guix archive --export --recursive`把引用的传递闭包连同 store item 元数据一起导出 | Endem 语义、成员权限、结果域和 Endem closure 的未来物理格式 |
...
| [Endem closure 提案向量](https://github.com/Noemion/noemion.github.io/tree/main/vectors/endem-closure) | 执行允许分类与确定拒绝的资料一致性检查 | 解析器、形成器、independent inspector、bounded runner、远程仓库或组件符合性 |

**当前策略：**CLOSURE-CORE 只定义抽象组合闭包和条件激活。动态清单不得改写闭包，成员结果不得合并，激活状态不得扩大权限。

**名称状态：**closure 是普通英语词，Endem closure 已经按逐词词首、职责和关键字语料接受。它仍只是设计对象，不表示组合器或物理格式已经实现；文档首次出现时继续先写“组合闭包”职责。

**待定内容：**物理容器、版本约束语法、符号与重定位模型、可选分支编码、执行顺序、并行调度、受限循环、远程对象仓库和跨会话缓存。当前尚未实现 deterministic producer、independent inspector、bounded runner、Endem closure 读取器或 CLI。
