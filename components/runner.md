...
contract 只保存本次运行需要的非秘密语义与能力描述，不包含 Bearer token、文件描述符、网络连接或其他实时句柄。SESSION-CORE 分别核对 `resolved` 内容、外部陈述、验证政策、截止点、撤销状态和依赖方准入，不把这些关系压成内容自身的布尔状态。完整契约见 [SESSION-CORE](../specifications/session-contract.html) 与 [ADR-0024](../architecture/adr-0024-session-contract.html)。
