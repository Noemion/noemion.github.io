...
nav_title: "新专题"
hero_title: "新专题"
hero_description: "面向读者的页面用途与直接结论。"
...
每个 Endem 按固定顺序表达 `say`、`mean`、`case`、`when` 与 `open`。`say` 保存来源记号，`mean` 保存已授权的符号与关系投影，`case` 保存一个根可能事态，`when` 保存满足观察与决定契约，`open` 保存仍未解决的投影缺口。五组是类型化规范语义，不是五段文本；旧 `aim/must/done` 不再是现行顶层字段。

ADR-0009 规定命题投影语义的底线：`mean` 与 `case` 必须共享稳定符号和可比较关系位置；逻辑形式由图结构显示，不设置自填的 `logical_form`、`valid` 或 `true` 字段；结构无意义、语义未决和观察未知分别使用 `no-sense`、`open` 与 `unknown`，不得互相代替。
...
1. 页面先回答读者正在问什么、为什么重要和直接结论，再引入 Endem、`say/mean/case/when/open`、Weave、Frame、Witness、Core、Reader 和 Runner 等项目术语。
