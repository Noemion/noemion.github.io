...
- 64 个正式页面由 Jekyll 处理：页面源码保存 YAML Front Matter 与 `<main>` 正文，公共 `<head>`、站点头部、目录容器和页脚由自定义布局生成；不添加主题覆盖现有 GNU/sourceware 视觉，也不创建 `.nojekyll`。
...
运行测试前必须安装可从命令行调用的 Node.js。源码阶段验证 64 个页面的 Front Matter、固定路由、共享布局和目录注册；Jekyll 构建后，质量测试会直接检查 `_site` 中的最终 HTML，并加载同一份生产 `assets/directory.js`，验证模块覆盖、页面类型、工具文档高亮以及模块不会向全局对象暴露 API。
