...
1000px 以下统一使用移动目录，避免 iPhone 14 Pro 等设备横屏时落入依赖悬停、无法隔离背景滚动的桌面下拉导航。非首页把面包屑与导航按钮固定在同一第二行；目录可通过再次点击、点击外部或 `Escape` 关闭，并以 180ms 淡出、模糊和轻微上移动画收回。外部关闭只响应完成的点击，不响应滑动开始，避免屏幕边缘的滚动手势误关目录。首页没有面包屑，继续使用单行移动头部。

移动目录打开时冻结并记录根页面位置，只允许固定在实时视觉视口内的目录面板纵向滚动；Safari 工具栏、方向或可视高度变化会同步更新面板高度。同步守卫在捕获阶段拦截面板外手势、首次未跟踪的触摸移动和顶部或底部继续滑动，不能把滚动链传给后方正文。滚动锁在关闭动画完成后解除并无动画恢复原位置。门户按钮的文字颜色不继承普通链接的已访问颜色，访问前后保持各主题规定的前景色。
