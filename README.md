...
ADR-0034 把目标语言读音和词表内口头区分加入发行名称门禁。职责、书写检索、读音稳定和听辨必须分别通过；现行职责保持有效，但具体发行拼写与读音尚未冻结。`Iknem`、`Ktisor/ktise`、`sphra`、`kine/mene` 和 Endem/Synem 的成对区分优先复核。读音提示不是命令别名，TTS、ASR 或语音模型输出也不能替代人类证据、授权或最终决定。
