...
- 65 个正式页面由 Jekyll 处理：页面源码保存 YAML Front Matter 与 `<main>` 正文，公共 `<head>`、站点头部、目录容器和页脚由自定义布局生成；不添加主题覆盖现有 GNU/sourceware 视觉，也不创建 `.nojekyll`。
- 本地可在安装兼容 Ruby 与 Bundler 后运行 `bundle install` 和 `bundle exec jekyll serve`；当前工作区自带的系统 Ruby 不作为发布环境基线。

## Jekyll 源码模型

- `_layouts/default.html`：唯一的全站 HTML 外壳，负责语言、标题、共享 CSS/JavaScript、页面角色和正文插槽。
...
- 每个正式 `.html`：使用 `layout`、`title`、`page_role`、`footer_text` 与 `permalink` 元数据，只编写该页面职责对应的 `<main>` 正文。
...
`index.html` 作为项目门户采用单独的叙事型首页设计，布局节奏、内容编排和交互动效深度参考 [AIHero 首页](https://www.aihero.dev/)：使用居中的长页面画布、首屏左右叙事、论点双栏、章节标题、横向入口行与网格内容交替组织。该参考只用于复刻信息层级、空间节奏和交互手感；页面继续使用 Noemion 自有内容、图形、配色和状态语言，不复制 AIHero 的人物、课程、品牌资产或正文。门户页使用顶部导航替代桌面固定侧栏，内页继续保持 GNU/sourceware 文档布局；首屏进入、对象图形、入口行和移动目录必须支持减少动态效果设置。首页详细设计、内容映射、动效约束和验收标准以 [`homepage-aihero-design.md`](homepage-aihero-design.md) 为准。
