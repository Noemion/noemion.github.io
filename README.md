...
移动目录打开时冻结并记录根页面位置，只允许目录面板自身纵向滚动并阻止滚动链传递到后方正文；菜单高度使用动态视口，滚动锁在关闭动画完成后解除并无动画恢复原位置。门户按钮的文字颜色不继承普通链接的已访问颜色，访问前后保持各主题规定的前景色。
...
ADR-0034 把目标语言读音和词表内口头区分加入发行名称门禁。职责、书写检索、读音稳定和听辨必须分别通过；现行职责保持有效，但具体发行拼写与读音尚未冻结。`Iknem`、`Ktisor/ktise`、`kine/mene` 和 Endem/Synem 的成对区分优先复核。读音提示不是命令别名，TTS、ASR 或语音模型输出也不能替代人类证据、授权或最终决定。
