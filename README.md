...
ADR-0013 保留 END-P0 作为结构实验，并建立 profile 2 的 END-P1 作为首个实现 Profile。END-P1 固定六个记录的字段键、嵌套 map、枚举、规范数组顺序与引用闭包；当前 7 个完整载荷字节向量覆盖 2 个语义接受和 5 类确定性拒绝，其中包含首个非空 `apor`。它仍不是稳定 ABI。
