...
- 现有 HTML、CSS、JavaScript 和资源继续作为 Jekyll 静态文件发布；不添加主题覆盖现有 GNU/sourceware 视觉，也不创建 `.nojekyll`。
...
- `tools/<tool>/docs/`：仅在存在实质文档时建立；`docs/index.html` 是文档中心，其他 HTML 是专题页。
- `docs/`：保存入门、使用、架构、开发、工具和规范六类任务型文档；正文链接权威页面，不复制第二套规范。
- `downloads/`、`faq/`、`development/`、`news/`：资源状态、常见问题、开发入口和经确认的进展。

## 浏览与共享资源

从 `index.html` 开始浏览。所有 HTML 使用深度正确的相对链接，并且只加载共享资源。`assets/style.css` 统一导入 `assets/directory.css`；目录的基础布局、折叠、高亮、响应式行为和滚动条在共享 CSS 中维护。`assets/directory.js` 同时承担目录渲染引擎和模块配置中心，根据当前路由选择项目、架构与对象、文档、资源、开发、工具、单个工具或工具文档目录。HTML 只保留统一的空目录容器，不复制导航链接。

导航一致性以“可达”而不是“完全相同”为准：每个非门户页面至少提供返回直接上级 `index.html`、所属模块目录页或路由表登记的特定目录页的方式。面包屑、模块目录中的上级入口以及手册导航中的“上级”都可以承担这一职责。

目录交互借鉴 AIHero 资源导航的分组信息层级：每组显示序号、名称和入口数量，当前页面所在分组自动展开，箭头和内容面板采用约 180–200ms 的快速过渡，链接状态采用约 150ms 的过渡；视觉仍保持 Noemion 的 GNU/sourceware 风格，并支持减少动态效果设置。

## GNU/sourceware 风格基线

GNU/sourceware 是全站的信息组织与视觉基线，而不是要求所有页面复制同一模板。页面应以正文、目录、稳定链接和清楚的层级为中心，保持克制、耐读、低装饰和长期可维护；避免营销式大图、无信息价值的卡片堆叠、过度动效以及把所有内容压成同一种落地页。桌面端的侧栏与正文作为一个整体居中，屏幕两侧保留可见但克制的空白，不使用贴边或全宽铺满布局；当前基线为约 28px 最小外侧留白、约 1480px 整体上限和约 1000px 正文上限。

全站统一品牌、字体、颜色、基础间距、面包屑和目录引擎；具体目录分组、正文结构、表格、清单、手册分页和状态展示由模块职责决定。模块可以共享布局组件，但不得为了视觉对称强行增加不适用的章节。

运行 `python3 tests/site_quality_test.py` 前必须安装可从命令行调用的 Node.js。质量测试会由 Python 直接加载同一份生产 `assets/directory.js`，验证 64 个路由均被模块目录覆盖、页面类型能选择正确目录、工具文档子树的高亮行为以及模块不会向全局对象暴露 API。
...
本 README 是站点结构、页面角色、正式路由和维护规则的唯一登记来源；不要在项目级 `AGENTS.md` 中复制路由表或页面级维护细节。下表是机器可读的完整 HTML 注册表，每个实际 HTML 文件必须且只能出现一次。
...
2. 全局颜色和正文排版只编辑 `assets/style.css`；目录样式只编辑 `assets/directory.css`。禁止页面级 CSS、内联 `<style>` 或 `style=`。
3. `assets/directory.js` 维护目录渲染和全部模块配置；模块目录可以拥有不同的内容与分组，但必须覆盖注册表中的全部 HTML 路由，并为每个页面选择正确的模块目录。新增模块目录时继续使用同一共享引擎，不在 HTML 中复制链接。
...
7. 每次路由或页面角色变更运行 `python3 tests/site_quality_test.py`，确认实际 HTML、README 注册表、全局目录和页面角色一致。
