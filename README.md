...
软件 Agent 身份与责任链的当前边界记录在 [`spec/software-agent-identity-and-accountability-boundaries-proposal.md`](spec/software-agent-identity-and-accountability-boundaries-proposal.md)。模型身份、Agent 定义、部署、工作负载、运行实例、会话、凭据、主体委托和一次动作分别定位不同事实；服务账户、工作负载认证或 Agent Card 都不能替代实际行动者、被代表主体与逐动作授权。该提案不创建 Agent 身份 CORE、制品、目录、服务、命令、组件或新专名，只有用户接受责任分配后才可能把唯一义务归还现有 ID、AUT、DRO、ADP、IKN、DIA 与 TEXT-IDENTIFIER。
