...
- `spec/*.md` 保存版本化规范条款，`spec/registry.json` 保存机器可读成熟度与验证映射，`vectors/` 保存规范向量。这些工程源文件由 Jekyll 排除；公开规范页面负责直白解释并链接 GitHub 中的精确源文件，不能复制另一套条款。
...
规范检查运行 `python3 tests/spec_contract_test.py`，源码页面检查运行 `python3 tests/site_quality_test.py`。完整发布检查随后运行 `bundle exec jekyll build`，执行 `cp sitemap.md _site/sitemap.md`，再运行 `python3 tests/site_quality_test.py _site`；GitHub Actions 按同一顺序验证并部署。构建产物中的手册目录数据按正式路由排序，不能让文件系统或 Jekyll 页面枚举顺序改变同一提交的输出字节。
...
- `spec/` 与 `vectors/`：实现可逐条引用的 Markdown 规范源、机器可读登记和正反向量；它们不生成并列 Pages 路由，也不冒充已冻结线格式。
...
10. `vectors/` 中的 JSON 只承载规范测试外壳，不是 `.endem` 线格式。每个向量必须固定规范版本、预期结果、条款 ID 与拒绝位置，并通过 `tests/spec_contract_test.py` 校验。
