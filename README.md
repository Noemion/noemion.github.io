...
10. `vectors/semantic/` 中的 JSON 只承载语义测试外壳，不是 `.endem` 物理格式；`vectors/wire/` 中的十六进制表达 END-FMT 实验字节。每个向量必须固定规范版本、预期结果、条款 ID 与拒绝位置，并分别通过 `tests/spec_contract_test.py` 和 `tests/wire_vector_test.py` 校验。
