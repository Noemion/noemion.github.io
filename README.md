...
ADR-0013 保留 END-P0 作为结构实验，并建立 profile 2 的 END-P1 作为首个实现 Profile。END-P1 固定六个记录的字段键、嵌套 map、枚举、规范数组顺序与引用闭包；当前 4 个完整载荷字节向量实际覆盖 1 个语义接受、未知字段、悬空引用和来源范围越界。它仍不是稳定 ABI。
