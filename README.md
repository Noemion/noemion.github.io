...
从 `index.html` 开始浏览。Jekyll 布局通过 `relative_url` 统一加载共享资源，正文内部继续使用与正式路由对应的相对链接。`assets/style.css` 与 `assets/directory.css` 由共享布局加载并携带同一构建版本；目录布局、折叠、高亮和响应式行为只在 `assets/directory.css` 维护。一级导航与手册侧栏在构建期成为可直接访问的 HTML；移动端同步守卫在目录控制器载入前后都负责页面冻结和手势边界，控制器建立后只接管菜单状态。`assets/site.mjs` 在 1000px 以下页面启动时准备纯文字模块目录，在桌面悬停或聚焦、横屏平板主动打开菜单时才加载带封面的二级导航，正文增强仍按内容需要加载。模块职责与失败降级见 [`design-system/frontend-architecture.md`](design-system/frontend-architecture.md)。

导航一致性以“可达”而不是“完全相同”为准：每个非门户页面至少提供返回直接上级 `index.html`、所属模块目录页或路由表登记的特定目录页的方式。面包屑、模块目录中的上级入口以及手册导航中的“上级”都可以承担这一职责。

全站顶部导航采用项目任务分组：桌面一级入口在悬停或键盘聚焦时展开纵向内容面板，先说明分组用途，再显示带专属 SVG 封面的入口卡片；移动端只复用同一目录数据和文字，不创建、请求或合成这些不可见封面。达到桌面宽度但仍使用无悬停粗指针的横屏平板，初始状态同样不加载封面；第一次触摸一级入口时才按需建立即将显示的富菜单，第二次触摸标题可进入目录，也可直接选择菜单卡片。触摸产生的焦点本身不提前加载资源，外接键盘使用 Tab 聚焦时仍可展开完整菜单。箭头和内容面板采用约 180–220ms 的快速过渡，卡片按 45ms 错峰进入，并支持减少动态效果设置。

1000px 以下统一使用移动目录，避免 iPhone 14 Pro 等设备横屏时落入依赖悬停、无法隔离背景滚动的桌面下拉导航。非首页把面包屑与导航按钮固定在同一第二行；目录首次点击即开始与图标同步的 180ms 展开动画，不等待模块、数据或动画延时；再次点击、点击外部或按 `Escape` 时立即开始反向动画。外部关闭只响应完成的点击，不响应滑动开始，避免屏幕边缘的滚动手势误关目录。首页没有面包屑，继续使用单行移动头部。
...
ADR-0035 把公开动作收敛为 `ktise`、`elenk`、`pleko`、`theor` 与 `drase`。派生封装由对应制品生产者在 Profile 冻结后承担；签名保持为独立外部集成；差分、恶意输入、复现和互操作检查保持为内部符合性门禁。三类责任不再占用公开动作名，也没有兼容别名。
...
语义等价与迁移的当前边界记录在 [`spec/semantic-equivalence-and-migration-proposal.md`](spec/semantic-equivalence-and-migration-proposal.md)。精确身份、封闭结构同构、有范围观察等价、迁移、强化/弱化和模型相似度必须分开；当前没有通用裁剪动作或裁剪等价，无法依据冻结的保留关系证明允许变换时必须保留或失败。该提案不创建 Semantic Key、等价制品、命令、组件或 CORE，只有用户接受责任分配后才可能把唯一义务归还现有规范。
...
8. 正式 HTML 面向开发者、普通用户和潜在使用者，只解释项目、产品、规范、使用场景与真实状态；不得出现用户指令与授权过程、内容生成、内部 review、补写过程、建站要求、页面制作、资料检查器或测试驱动文案等幕后信息。阶段限制改写为“当前没有组件实现”等客观状态；状态说明使用“当前策略、已有成果、正在研究、待定内容、限制条件”，不使用“采用决定、未决边界、冻结、门禁”等治理话术，正式字段或不可替代的技术术语除外。
