...
> 当前状态：ADR-0037 已采用 Endem closure、session contract、evidence entry、deterministic producer、independent inspector、bounded runner 与 `form / lint / compose / inspect / run`。以下内容保存采用前的专名必要性与候选比较；其中 `check` 因 PostgreSQL SQL 关键字冲突改为 `lint`。现行词表以 [ADR-0037](../architecture/adr-0037-terminology-simplification.html) 为准。
...
验证必须遵守 [ADR-0034](../architecture/adr-0034-pronunciation-and-oral-distinction.html) 的两阶段要求。首轮材料至少包含：
