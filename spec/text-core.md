...
本规范定义 Endem、Synem、Dromen、Tekmor、诊断、外部适配和模型候选怎样区分原始来源文本、结构标识符、登记词、描述文本与显示视图。它回答“机器实际取得哪些码点、哪些变换已经发生、两个字符串按什么规则比较，以及人和模型看到的是否是同一份内容”。

`TXT-CORE` 不是自然语言理解规范，也不创造语义等价、Unicode 标识符或文本制品。相同字形不表示相同字节、相同码点或相同 `semion`；不同字形也不自动表示不同意义。当前 END-P1 的结构标识符继续只允许 ASCII。来源文本可以使用 Unicode，但任何规范化、大小写折叠、双向重排、同形检测和不可见字符处理都必须保持在自己的职责层。

文本用于授权、同意或多人决定时，安全显示视图和精确机器请求还必须固定并符合 `AUT-CORE` 的精确版本。TXT-CORE 只证明显示与机器内容怎样对应，不证明观看者具有决定权。
...
**验证：**`TXT-SCN-001`；`vectors/text/cases.json`；未来 `peira:text-slot-classification` 组件测试。

### TXT-ENC-001 — 解码必须唯一且先于任何变换
...
**验证：**`TXT-SCN-002`、`TXT-SCN-003`；`vectors/text/cases.json`；未来 `peira:utf8-unique-decode` 组件测试。

### TXT-SRC-001 — 原始字节、解码文本与变换结果必须分开
...
**验证：**`TXT-SCN-004`、`TXT-SCN-005`；`vectors/text/cases.json`；未来 `peira:source-byte-text-provenance` 组件测试。

### TXT-IDN-001 — 结构标识符保持封闭 ASCII
...
**验证：**`TXT-SCN-006`、`TXT-SCN-007`；`vectors/text/cases.json`；未来 `peira:ascii-identifier-closure` 组件测试。

### TXT-NRM-001 — 规范化必须绑定槽与版本
...
**验证：**`TXT-SCN-008`、`TXT-SCN-009`；`vectors/text/cases.json`；未来 `peira:text-normalization-profile` 组件测试。

### TXT-CMP-001 — 比较必须声明比较域
...
**验证：**`TXT-SCN-010`；`vectors/text/cases.json`；未来 `peira:text-comparison-domain` 组件测试。

### TXT-RNG-001 — 范围必须绑定主体、单位与变换
...
**验证：**`TXT-SCN-011`、`TXT-SCN-012`；`vectors/text/cases.json`；未来 `peira:text-range-unit-binding` 组件测试。

### TXT-BID-001 — 存储逻辑顺序与显示顺序必须分开
...
**验证：**`TXT-SCN-013`、`TXT-SCN-014`；`vectors/text/cases.json`；未来 `peira:bidi-storage-display-separation` 组件测试。

### TXT-HID-001 — 不可见字符与同形风险不能静默处理
...
**验证：**`TXT-SCN-015`、`TXT-SCN-016`；`vectors/text/cases.json`；未来 `peira:hidden-confusable-inventory` 组件测试。

### TXT-MET-001 — 语言与媒体元数据只是声明
...
**验证：**`TXT-SCN-017`；`vectors/text/cases.json`；未来 `peira:text-metadata-locale` 组件测试。

### TXT-AIM-001 — 模型输入必须绑定机器实际接收的文本
...
**验证：**`TXT-SCN-016`、`TXT-SCN-018`；`vectors/text/cases.json`；未来 `peira:model-text-view-binding` 组件测试。

### TXT-OUT-001 — 显示、复制与导出必须披露变换
...
**验证：**`TXT-SCN-018`；`vectors/text/cases.json`；未来 `peira:text-display-export-provenance` 组件测试。
