---
layout: spec
title: "Noemion 生命周期与结果词边界研究提案 · Noemion"
page_role: "content"
...
这些只是候选方向。现行规范、向量与页面在人类验证和迁移 ADR 完成前继续使用当前值。
...
| --- | --- | --- | --- |
| A · 对象边界 | 从内容形成分类移除 `attested`，把外部陈述、验证结果和依赖方判断显式分开 | ID-CORE 的不可变身份边界、RFC 9334 的角色与消息分层、in-toto 的主体与包络分层、SLSA 的逐项验证要求，以及多政策、撤销和截止点反例 | 形成迁移 ADR；同步 END-CORE、DRO-CORE、登记、威胁、场景和向量 |
...
[in-toto Statement v1](https://github.com/in-toto/attestation/blob/main/spec/v1/statement.md)进一步把陈述建模为按摘要绑定一个或多个不可变主体，并以 `predicateType` 区分陈述类型。主体、陈述内容与认证包络因此是三层不同责任；签名包络存在不能替代主体摘要和陈述类型匹配。

[SLSA v1.2 的制品验证规范](https://slsa.dev/spec/v1.2/verifying-artifacts)要求验证者分别检查来源包络签名、陈述主体摘要、谓词类型、构建者信任根和调用方期望。不同验证者还可以采用不同信任根与期望。它给出了直接反例：同一份内容和同一份来源陈述，在不同调用方政策下可以得到不同适用结论，因此不能固化成内容自身的 `attested` 状态。
...
| 外部陈述集合 | 每项陈述的完整身份、类型、精确主体摘要、谓词或声明、包络和生产者 | 单一布尔值截断多签、多主体、多陈述或陈述类型 |
| 验证记录 | 验证者、信任根、验证政策、参考值或期望、实际检查、结果和失败原因 | “签名存在”自动变成“主体匹配且声明适用” |
...
| 签名与主体摘要都匹配，但陈述类型不是建立政策要求的类型 | 拒绝 | 一个有效漏洞扫描陈述不能替代来源、授权或其他必需声明 |
| 同一陈述通过验证者 A 的信任根和期望，但不满足验证者 B | 分别保存两项验证记录；B 不得复用 A 的结论 | 验证适用性依赖具名信任根、期望与政策，不是内容固有属性 |
...
验证遵守 [ADR-0034](../architecture/adr-0034-pronunciation-and-oral-distinction.html)，并使用两批独立参与者。发现阶段至少包含：
...
若 A 轴形成正式决定，迁移 ADR 应只纠正对象与关系，不等待 B 轴选出新名称，也不把候选名称写入接口：
...
3. 以外部 typed signed statement、attestation result、验证记录、信任根、政策、期望、截止点和撤销状态替换 Dromen 的 `subject.attested` 布尔值；同步 DRO-SUB-001、DRO-IMM-001、威胁模型、场景与全部提案向量。
4. 为单项有效陈述、多签、签名者无权、主体不匹配、陈述类型不匹配、验证者政策分歧、过期、撤销、缓存结论和外部状态升级分别建立正反向量；不得只测试“布尔值为真”。
...
- 规范引用与向量检查全部通过，且公开说明不把资料一致性描述成组件实现。
...
本提案现在形成一项尚未进入规范的边界结论，以及四项仍待人类验证的命名结论：
...
这些结论不会改变任何现行规范值。A 轴可以单独形成对象边界迁移 ADR；这不会使 `formed / resolved` 等 B 轴候选词自动成立。B 轴何时开始人类验证、采用哪些名称以及是否形成后续迁移 ADR，仍是待定内容。
