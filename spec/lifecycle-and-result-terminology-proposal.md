...
| --- | --- | --- | --- |
| A · 对象边界 | 从内容形成分类移除 `attested`，把外部陈述、验证结果和依赖方判断显式分开 | ID-CORE 的不可变身份边界、RFC 9334 的角色与消息分层、in-toto 多签包络，以及多政策、撤销和截止点反例 | 形成迁移 ADR；同步 END-CORE、DRO-CORE、登记、威胁、场景和向量 |
