...
**要求：**END-P1 中的 `projection.kind`、`projection.id`、`decision_authority` 与 `required_tekmor` `MUST` 只解释为内容要求中的有限选择器；它们不内联授权决定、政策、权威目录、签名、Tekmor 实例或验证材料。读取器 `MUST NOT` 通过名称、当前环境、网络查询或默认目录把这些选择器提升为授权或证据已经存在。END-P1 单文件最多支持容器接受与 Profile 接受；完整 `END-CORE` 内容接受还 `MUST` 显式核对外部前置条件。
