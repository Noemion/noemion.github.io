...
- 实现状态：仅有实验字节与一致性检查工具；没有生产写入器或 inspector
...
END-P2 有三个 Profile 接受和十一项确定拒绝；接受结果统一报告为 `profile-accept / external-prerequisites-not-evaluated`，因为当前检查器只验证封闭字段、引用、来源范围和资源边界，没有核对 AUT-CORE 授权决定。当前一致性检查工具会读取这些向量，没有组件执行它们。
