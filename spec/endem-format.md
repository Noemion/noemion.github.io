...
END-P0 字节只覆盖前两层。`WV-STRUCT-ACCEPT-001` 中的六个空映射会被结构层接受，但语义层必须因内容缺失而拒绝；它不是最小有效 Endem。END-P1 另有两个语义接受、五个语义拒绝，以及非最短 CBOR、图边、深度和字符串超限拒绝；第二个接受向量实际保存非空 `apor`，首个 Rust 候选已同时执行这些向量。
