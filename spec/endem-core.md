...
本规范是 Endem 的通用内容标准。它定义一个 Endem 在进入物理编码之前必须满足的最小语义、授权、状态、身份和安全读取要求，并约束任何 Profile 能够表达什么、不得改写什么。它约束未来 Ktisor、生产读取器、独立 Theor 与规范向量，但不选择文本语法、二进制布局、魔数、字段宽度、记录编号、摘要算法、签名算法或稳定 ABI。
...
**要求：**Ktisor 对相同的规范化来源、授权决定、配置、依赖、Profile 与版本 `MUST` 产生相同的规范结果；线格式冻结后还 `MUST` 产生逐字节相同的 Endem。
