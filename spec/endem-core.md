...
本规范是 Endem 的通用内容标准。它定义一个 Endem 在进入物理编码之前必须满足的最小语义、授权、状态、身份和安全读取要求，并约束任何 Profile 能够表达什么、不得改写什么。它约束未来 Poiet、生产读取器、独立 Theor 与规范向量，但不选择文本语法、二进制布局、魔数、字段宽度、记录编号、摘要算法、签名算法或稳定 ABI。
...
**验证：**`tests/spec_contract_test.py` 的规范层次契约；未来 `peira:layered-conformance-report` 测试。
...
**验证：**`spec/profiles/end-p1.json`、`tests/p1_payload_test.py` 与 `tests/spec_contract_test.py`；未来 `peira:profile-semantic-closure` 测试。
...
**要求：**符合性报告 `MUST` 分开记录由当前 `.endem` 字节直接证明的容器与 Profile 结论，以及依赖外部规则、授权、身份、签名或 Tekmor 的内容结论。未取得、未执行、版本不匹配、已撤销或无法精确绑定的外部前置条件 `MUST` 标为未满足或未求值；`MUST NOT` 由文件内名称、环境搜索、缓存成功或调用者断言补齐。一个 Profile 若不能单独承载完整内容接受所需的前置条件，`MUST` 明确声明其单文件最高可声称层次。

**失败：**读取器把 `projection.id`、`decision_authority`、`required_tekmor`、签名者名称或本地 ACL 命中解释为外部义务已经完成，或在报告中省略未执行层次时，内容接受声明无效。

**验证：**`SCN-028` 至 `SCN-030`；语义向量的外部前置条件矩阵；`spec/profiles/end-p1.json` 的声明上限；未来 `peira:external-conformance-prerequisites` 测试。
...
**验证：**`SCN-024` 与 `SCN-025` 设计审查；`vectors/composition/cases.json`；未来 `peira:composite-root-boundary` 组件测试。
...
**验证：**`SCN-024` 与 `SCN-027` 设计审查；`vectors/composition/cases.json`；未来 `peira:composition-topology-alignment` 组件测试。
...
**验证：**`SCN-024` 与 `SCN-026` 设计审查；`vectors/composition/cases.json`；未来 `peira:four-result-composition` 组件测试。

### END-CMP-004 — 短路必须保存决定依据与求值覆盖

**要求：**只有在 `all_of` 已得到有效 `unmet` 或 `any_of` 已得到有效 `met` 后，求值才 `MAY` 短路。结果 `MUST` 保存决定依据、所有已求值叶及所用 Tekmor、未求值叶身份与停止原因。未求值叶 `MUST NOT` 被记录为 `agno`。不同求值顺序可以改变求值覆盖，但 `MUST NOT` 改变已经出现决定性结果后的总体分类；短路前已经观察到的故障 `MUST` 保留。

**失败：**在结果仍依赖未求值叶时提前停止、没有记录未求值叶、把未执行等同于观察不足、只保留总体布尔值，或通过先求值有利分支隐藏已发生故障时，该结果无效。

**验证：**`SCN-026` 设计审查；`vectors/composition/cases.json`；未来 `peira:decisive-short-circuit-provenance` 组件测试。
...
**验证：**`SCN-007`、`SCN-021` 与 `SCN-023` 设计审查；`vectors/measurement/cases.json`；未来 `peira:measurement-construct-and-population` 组件测试。
...
**验证：**`SCN-020` 与 `SCN-022` 设计审查；`vectors/measurement/cases.json`；未来 `peira:numeric-unit-conversion` 组件测试。
...
**验证：**`SCN-020`、`SCN-021` 与 `SCN-023` 设计审查；`vectors/measurement/cases.json`；未来 `peira:measurement-procedure-replay` 组件测试。

### END-MSR-004 — 比较边界与不确定度必须共同决定结果

**要求：**阈值契约 `MUST` 固定 `lt/le/gt/ge/eq` 比较器、阈值及单位和 `interval` 不确定度政策。观察的有效不确定区间完全落在满足一侧时才 `MAY` 产生 `met`，完全落在反驳一侧时才 `MAY` 产生 `unmet`；区间与边界重叠 `MUST` 产生 `agno`。点估计、仪表盘显示值、排名或置信度标签 `MUST NOT` 越过区间单独决定结果。比较结果 `MUST` 绑定原始观察、换算、聚合器、阈值、区间和实际使用的 Tekmor。

**失败：**`<` 与 `<=` 被视为相同、误差条被隐藏、点估计越线但区间跨线仍宣称满足、多个阈值中只报告通过项，或模型输出的自报置信度被当作统计不确定度时，该判断必须拒绝。

**验证：**`SCN-020`、`SCN-021` 与 `SCN-023` 设计审查；`vectors/measurement/cases.json`；未来 `peira:threshold-uncertainty-classification` 组件测试。
...
**验证：**`SCN-028` 与 `SCN-029`；`SV-REJECT-AUTHORIZATION-BINDING-001` 与语义接受向量；未来 `peira:projection-authorization-companion` 测试。
...
**要求：**对同一 Profile 与同一规范字节，授权主体、授权决定、签名、验证材料、撤销状态、Tekmor、满足结果和最终决定变化时，Endem 的精确内容身份 `MUST` 保持不变；变化只产生新的伴随关系或相对具名截止点的新评估。任何必须改变六个语义面或规范字节的语义修订 `MUST` 形成新的内容身份，不能伪装成授权状态更新。

**失败：**两个不同授权者确认同一字节却得到两个 Endem 内容身份，授权撤销导致历史字节身份变化，或改变语义字段后继续沿用旧身份时，身份或伴随关系无效。

**验证：**`SCN-030`；语义向量的内容身份等价组；未来 `peira:identity-independent-of-external-decisions` 测试。
