...
本规范不冻结诊断 JSON、CBOR、SARIF、Problem Details、JSON-RPC 或 CLI 文本编码，不定义进程退出状态、颜色、终端布局、网络媒体类型、持久日志、遥测 schema、堆栈格式、修复补丁语法或跨进程关联协议。它们必须由真实消费者、独立 ADR、威胁分析、正反向量与实现证据共同支持后才能进入稳定接口。
