...
**验证：**`IKN-SCN-001`、`IKN-SCN-002`；`vectors/iknem/cases.json`；未来 `peira:iknem-scope-binding` 组件测试。
...
**验证：**`IKN-SCN-003`、`IKN-SCN-004`；`vectors/iknem/cases.json`；未来 `peira:iknem-provenance-closure` 组件测试。

### IKN-OBS-001 — 观察、变换与主张必须分开

**要求：**观察类 Iknem `MUST` 把原始观察引用、规范化步骤、结构化 `phain` 和最终主张分开。`phain` `MUST` 与适用 `skena` 共享可比较的符号身份、关系身份、角色位置、极性和作用域。丢失、过滤、聚合、换算、舍入、脱敏与模型解释 `MUST` 记录为可定位变换及其损失。自由文本、工具成功状态、模型置信度或日志数量 `MUST NOT` 自行升级为关系事实。

**失败：**无法把观察对齐到关系位置、把推断伪装成原始观察、遗漏有损变换，或把空查询直接编码为负极性 `phain` 时，该观察主张无效。

**验证：**`IKN-SCN-005`、`IKN-SCN-006`；`vectors/iknem/cases.json`；未来 `peira:iknem-observation-alignment` 组件测试。
...
**验证：**`IKN-SCN-006`、`IKN-SCN-007`；`vectors/iknem/cases.json`；未来 `peira:iknem-evidence-class` 组件测试。
...
**验证：**`IKN-SCN-007`、`IKN-SCN-008`；`vectors/iknem/cases.json`；未来 `peira:iknem-integrity-separation` 组件测试。
...
**验证：**`IKN-SCN-008`、`IKN-SCN-009`；`vectors/iknem/cases.json`；未来 `peira:iknem-validity-appraisal` 组件测试。
...
**验证：**`IKN-SCN-010`、`IKN-SCN-011`；`vectors/iknem/cases.json`；未来 `peira:iknem-coverage` 组件测试。
...
**验证：**`IKN-SCN-012`、`IKN-SCN-013`；`vectors/iknem/cases.json`；未来 `peira:iknem-decision-separation` 组件测试。
...
**验证：**`IKN-SCN-011`、`IKN-SCN-014`；`vectors/iknem/cases.json`；未来 `peira:iknem-minimal-disclosure` 组件测试。
