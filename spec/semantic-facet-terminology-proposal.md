...
> 当前状态：ADR-0037 已采用 `source_expression / meaning_projection / situation / goal_direction / satisfaction_criteria / unresolved_meaning / structured_observation`。以下内容保存采用前的职责分析、候选比较和反例；现行词表以 [ADR-0037](../architecture/adr-0037-terminology-simplification.html) 为准。
...
验证遵守 [ADR-0034](../architecture/adr-0034-pronunciation-and-oral-distinction.html)，并同时比较“现行短词 + 直白解释”和“直白字段名 + 同一解释”。当前重点不是证明普通英语单词有词典读音，而是证明整组名称能恢复正确职责且不会诱导跨层推断。
