...
来源清单让已经获得授权的来源、投影和判断契约进入确定性 Ktisor。它同时保存解码并处理转义后的自然语言表达和结构化决定，但不负责自然语言理解。它只产生含来源的 END-P1 形成制品，不定义最终发布 Profile 或裁剪结果。模型、界面或人工流程只能位于清单上游；Ktisor 不调用模型补字段，也不把候选自动升级为确认语义。
...
**要求：**含来源的 END-P1 形成制品中，`rhem` 保存来源文本；`symbol/relation/situation/root/telis/krin` 只保存确定性规则或具名权威已确认的投影。存在多个可表达候选但尚无权选择时，输入者必须写入 `apor`。模型输出不能直接充当 projection、decision_authority 或通过结论。未来裁剪发布 Profile 必须另行定义来源分离、引用重写和允许损失，不能把本清单的 `rhem` 记录直接删去后沿用 END-P1 身份。
