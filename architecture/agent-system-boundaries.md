...
| 本次运行最多能做什么？ | contract / 外部协议适配 | 协议、对端、环境、能力交集、预算、凭据域和需要记录的观察 | 发现、协商与可调用性不能扩大已经固定的会话权限 |
...
3. 固定一次会话的环境、能力交集、预算、凭据域和需要记录的观察。
...
- [会话契约（contract）](../specifications/session-contract.html) — 查看一次运行怎样固定可用环境、能力上限、预算和需要记录的观察。
