...
后三种量词的 `k` 必须是非负整数。提前决定只减少不必要的求值，不降低证据要求；聚合结果仍须保存每个实际成员结果和对应 evidence。
...
现行十二个量化提案向量覆盖六个允许分类和六个确定拒绝，只验证 END-QNT 三条抽象条款。本决定不增加 END-P2 字段，也不表示 producer、runner、求值器或 CLI 已经实现。
