...
summary: 区分字节容器、内容 Profile 和通用内容标准，说明每一层通过后究竟能证明什么、不能证明什么。
decision_id: ADR-0023
page_heading: ADR-0023 · 文件能读 · 不等于内容成立
page_lead: 物理容器负责安全解释字节，内容 Profile 负责关闭字段和能力，通用内容标准负责约束目标语义。三层分别版本化、分别报告，任何一层都不能替下一层作出结论。
badges:
- 当前策略
...
第一版稳定标准之前，改变字段、编号或结果域就直接产生新草案版本，不保留含义漂移的别名。未知必需能力关闭失败；没有真实消费者和失败责任的字段不进入内容标准。
