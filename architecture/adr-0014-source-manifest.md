...
summary: 说明实验来源清单怎样记录已经确认的原文、解释和判断条件，再供未来 producer 形成含来源的 Endem；模型不能替人补写字段。
decision_id: ADR-0014
page_heading: ADR-0014 · 实验来源清单 · 输入与权威边界
page_lead: 实验来源清单用有限的逐行语法记录已确认原文、采用的解释和判断条件，再映射到含来源格式。该语法标识为 END-SRCM；它不允许模型直接生成规范对象。
badges:
- 当前策略
...
这份实验性来源清单只描述形成输入。它把解码后的自然语言和已确认结构写入固定映射，且只允许已经登记的字段；它不理解文本，也不决定动作权限。

| 层次 | 来源清单能够提供 | 不能据此声称 |
| --- | --- | --- |
| 来源表达 | UTF-8 文本、来源身份、版本和解码后的范围。 | 逐字节保留原始 `.ends` 文件。 |
| 意义投影 | 由确定性规则确认，或由具名主体确认候选意义、语义位置、适用范围和截止点；规范把后一种记录称为语义授权绑定。 | 形成器有权替主体选择意义。 |
