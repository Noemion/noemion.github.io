...
END-FMT、END-P0 与 END-P2 都是 `0.1.0-draft`，不承诺向后兼容。当前只有规范、机器可读 Profile 和已执行的正反向量；没有正式写入组件、inspector、稳定 ABI、MIME 类型、升级协议或发布 Profile。任何新增记录、字段、变换或状态都必须先给出具名消费者、威胁、正反向量和独立读取方案。
