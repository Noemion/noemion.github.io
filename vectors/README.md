...
`result-domains/`、`mene/`、`negation/`、`quantification/`、`measurement/`、`composition/`、`synem/`、`dromen/`、`tekmor/`、`diagnostics/`、`adapters/`、`identity/` 与 `text/` 分别保存判断结果、时间连续性、否定缺席、量化成员资格、测量阈值、复合判据、Synem 闭包激活、Dromen 会话契约、Tekmor 证据评估、跨对象结构化诊断、外部协议适配、精确内容身份与签名以及文本与标识符边界的提案矩阵。它们用于检查设计规则之间是否一致，不是 `.endem`、Synem、Dromen、Tekmor、诊断、协议、签名或 Unicode 处理物理字节，也不证明运行组件存在。`identity/` 中的 SHA-256 只用于已知算法示例，不是发行算法决定；`text/` 只描述提案属性，不是解码器、规范化器、双向显示器、同形检测器或模型输入网关。
