...
`wire/p2/` 保存 END-FMT 0.1.0-draft 与 END-P2 的规范十六进制字节，使用完整六记录载荷并实际运行结构、字段、排序、引用和来源范围判断。这些向量不是稳定 ABI。
...
- 线格式向量必须固定 END-FMT 与精确 Profile。END-P2 由 `tests/p2_payload_test.py` 从语义源确定性编码、逐字段解码并比较预期。禁止只比较预生成摘要。
