...
工具按“规范与对象 → 文本 IR → 编译链接 → 发布运行 → 模型工程”逐步推进。每个工具必须说明上游输入、下游消费者、关键不变量、失败边界、阶段门和验证证据，不能只用一行命令描述代替设计。完整分类入口见[工具目录](../tools/)。
...
**当前范围：**[noemld 文档](../tools/noemld/docs/)已经按契约、输入输出、调用、流程、符号、重定位、SSO、安全、诊断、测试、依赖和术语索引拆分。其他工具目前提供职责、边界和阶段门说明，尚无独立操作手册。
