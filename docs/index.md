---
layout: "manual"
title: "文档 · Noemion"
page_role: "section"
footer_text: "Noemion · 文档"
permalink: "/docs/index.html"
manual_id: "docs"
manual_group: "start"
manual_order: 0
nav_title: "文档"
hero_title: "文档"
hero_description: "按任务查找入门、使用、架构、开发、工具文档和规范参考。"
summary: "按任务查找入门、使用、架构、开发、工具文档和规范参考。"
badges: ["Guides", "Tool Docs", "Reference"]
manual_is_index: true
manual_index_heading: "按任务查找"
---

## 如何使用这些文档

先根据任务建立共同背景，再进入权威架构、规范或工具专题。任务型文档解释概念和阅读路径，不复制第二套技术定义；看到“设计提案”“开放问题”或“未来阶段”时，不应把内容理解为已冻结接口。

推荐顺序：确认问题 → 阅读背景 → 定位权威页 → 核对成熟度 → 追踪证据。

## 文档边界

六类任务文档负责解释概念、提供阅读路径并说明当前可用性；架构关系由架构专题定义，规范性术语和约束仍以对应规范及经批准 ADR 为准。

## 成熟度与引用纪律

研究、论文、专利、软著和标准提案引用 Noemion 时，必须记录路由、版本或提交标识、成熟度和对应权威来源。项目愿景不能作为实验结论，候选类型名不能作为冻结 ABI，规划中的流程也不能作为已经发布的软件能力。

关键主张只有同时关联规范条款、ADR、测试、基准、数据来源和失败案例，才具备可审计的证据链；缺少证据的内容必须保持为设计提案或开放问题。
