...
软件工程长期把程序员写的形式代码编译给机器。人工智能系统现在可以接收普通语言、调用工具并改变外部系统，但人的目标通常仍留在临时对话和模型解释中。

Noemion 研究普通人怎样把自然语言表达的目标，形成供人工智能系统安全使用的工程制品。形成版保留原始表达、意义确认和未决问题；最终发布版移除原文，只保留经过确认的目标结构，并在使用前核对授权边界。这里的“编译”不是自然语言生成代码，而是让意义、约束、期望结果和接受条件进入一条可复核路径。

Noemion 不替代 Prompt、Agent 工作流、MCP、A2A、身份系统或政策引擎。这些系统负责生成、编排、通信或授权；Noemion 负责固定“究竟要达到什么”和“依据什么接受结果”。当前成果是规范、实验格式和安全边界，还没有可安装编译器或组件。
...
这里的“安全”不是文件自带的布尔属性，也不表示制品可以自行授权执行。开发者必须按下面的顺序核对；后一步不能弥补前一步的失败。

| 检查点 | 开发者核对什么 | 检查失败后怎样处理 |
| --- | --- | --- |
| 形成与发布 | 形成版是否保留原始表达、映射和未决项；发布 Profile 是否移除原文、产生新内容身份，并只保留经过确认的目标结构与必要来源绑定 | 无法说明裁剪关系或仍有未决意义时，不形成可使用的发布制品 |
| 内容完整性 | 实际字节的摘要、外部陈述的主体、签名、信任根、政策和截止点是否匹配 | 把内容或陈述视为不适用；不能用“文件能打开”继续运行 |
| 意义确认 | 具名权威是否针对精确来源、候选和语义位置确认了这一解释，剩余问题是否留在 `apor` | 不把模型候选写成确认意义，也不生成规范目标字节 |
| 动作授权 | 当前主体、对象、动作、目的、范围和截止点是否得到 `grant`；实际能力是否仍在会话上限内 | `deny / defer` 或能力不相交时不启动动作；已经运行则停止扩大影响并保留事实记录 |
| 证据与满足 | 有效记录是否覆盖 `krin` 要求的主体、关系、时间和观察责任，比较结果是 `met / unmet / agno / fault` 中哪一个 | 覆盖不足返回 `agno`，观察故障返回 `fault`；工具成功不能据此改写为 `met` |
| 最终决定 | 具名权威是否依据满足结果、证据限制和决定规则作出 `accepted / rejected / deferred` | 没有决定权威或依据不足时保持 `deferred`，不得由模型、运行器或证据记录自行接受 |

[SLSA 1.2 的制品验证](https://slsa.dev/spec/v1.2/verifying-artifacts)要求验证签名、陈述主体、信任根和调用方预期；这些检查可以发现内容或溯源不匹配，却不证明目标含义正确或动作已经获准。[RFC 9334](https://www.rfc-editor.org/rfc/rfc9334.html)把 Evidence、验证者形成的 Attestation Results 和依赖方自己的决定分开；Noemion 借鉴这条责任分层，不采用其远程证明协议或消息格式。

[W3C PROV-DM](https://www.w3.org/TR/prov-dm/)把派生写成一个实体经活动形成另一个实体。形成版与裁剪后的发布版因此保留不同身份和显式关系，但发布版不能据此还原已经移除的原始自然语言。[GNU Guix 的 `guix challenge`](https://guix.gnu.org/manual/en/html_node/Invoking-guix-challenge.html)比较独立构建的逐位结果；差异仍要继续判断来自不可复现、损坏还是恶意来源，不能直接决定目标意义、动作授权或最终接受。
...
| 已确认意义投影 | `semion` | 未经规则或具名权威确认的模型猜测 |
...
规范把 `semion` 的外部确认称为语义授权。它只回答“谁可以确认来源中的这个意义候选”，不回答“谁可以调用工具、修改仓库或部署服务”。动作授权必须针对精确主体、对象、动作、目的、范围和截止点另行形成；会话能力与最终决定随后继续分开。
