...
Noemion 正在定义一种可以持久保存、组合和独立检查的自然语言目标制品。它不替代 Prompt、Agent 工作流、MCP、A2A、身份系统或政策引擎；这些系统负责生成、编排、通信或授权，Noemion 负责固定“究竟要达到什么”和“依据什么接受结果”。当前成果是规范、实验格式和安全边界，还没有可安装组件。
...
- **[Endem](../specifications/endem.html)** 回答“一个目标究竟是什么”：它是最小目标制品。
- **[Synem](../specifications/synem.html)** 回答“多个目标怎样形成完整依赖范围”：它只在两个或更多 Endem 确实需要共同解析、发布或验收时存在。
- **[Dromen](../specifications/dromen.html)** 回答“这一次运行允许做什么”：它由 Drasor 从精确制品、政策、环境、能力、预算和证据责任封存，不是文件或可恢复权限。
- **[Iknem](../specifications/iknem.html)** 回答“什么观察支持哪项结论”：它绑定目标、环境、方法和声明范围，不是数学证明，也不会自动作出最终决定。

## 计划中的命令入口

如果项目进入实现，命令行只提供一个入口 `endem`。五个动作分别负责形成、检查、组合、独立查看和受限运行；当前没有可执行命令或稳定参数。
...
5. [开发指南](development-guide.html)：了解首个可实现范围、Ktisor/Theor 差分和验证条件。

## 当前状态

> 项目已经定义主要职责和计划中的应用结构；具体发行拼写和读音仍需完成 ADR-0034 的人类验证。规范编码和软件实现尚未发布；目前没有可安装的 `endem`、稳定扩展机制、ABI 或正式软件版本。

若进入实现，首个范围只包含 `ktise`、`elenk`、独立 `theor` 和必需的一致性验证。组合、发布、受控运行和模型适配仍需要前一范围的安全与复现证据。
