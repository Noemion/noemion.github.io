...
数据依次经过来源或 Horizon Engine、Noesis Core、Noema IR、Noema Object、链接器、装载器和 Fulfillment Runtime。每一步只能增加经过验证的结构或信任，不能绕过前一步的失败。
...
6. Fulfillment Runtime 求解目标并产生具体结果，不回写已经签名的对象。
