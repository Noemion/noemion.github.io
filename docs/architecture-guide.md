...
来源表达 + 已确认的意义
          │
          ▼
        producer ──► Endem ──► closure 组合闭包
                     │
                     └── inspector  当前只独立读取 Endem 字节

精确发布输入 + 当前授权 ──► runner ──► contract
                                      │
                                      ▼
                              受限动作与环境观察
                                      │
                                      ▼
                         evidence ─支持─► 满足结果 ─► 具名决定
```

Endem 保存一个根目标；closure 固定多个 Endem 的完整依赖闭包；contract 只约束一次受限会话；evidence 保存有范围的观察、方法、来源和限制。当前只有 Endem 具有实验性物理字节规范，图中的 closure、contract 和 evidence 不表示已经存在文件格式或组件实现。
...
| 固定目标内容 | producer 与 Endem | 模型建议仍是候选；只有确定性规则或具名意义决定可以进入制品 |
| 检查实际字节 | 生产侧 `lint` 与 inspector | 两条路径分歧时保存最小复现，不能改写输入制造一致 |
| 准备一次运行 | runner 与 contract | 对象、政策、环境、能力、预算或观察责任未闭合时，不建立会话 |
| 调用工具或远端 Agent | 控制平面与协议适配 | MCP/A2A 状态保留外部来源；`completed` 不直接映射为满足结果 |
| 形成环境事实 | `structured_observation` 与 evidence | 工具返回、日志或模型说明缺少主体、方法、范围或限制时，不能扩大主张 |
...
| 能力与预算 | 只分配当前 contract 的严格子集，共享或细分总预算 | 重新取得最小凭据和实时句柄，不从检查点反序列化权限 |
| 当前有效性 | 下游开始前重新核对对象、授权、协议和截止点 | 逐项重验适用的外部陈述、验证记录和依赖方准入判断 |
| 返回内容 | 下游完成仍只是候选运行事实，满足与接受继续独立判断 | 保存的控制位置只说明此前认为进行到哪里，不证明动作完成 |

恢复实现应先**选择一种对话状态策略**；必须组合本地历史与服务端状态时，按消息身份和截止点去重。**恢复必须重新验证**，并建立新的 contract，而不是复活旧会话。精确条款见[权威与授权决定规范](../specifications/authority.html)，研究边界见[软件 Agent 身份、委托与责任链提案](https://noemion.github.io/spec/software-agent-identity-and-accountability-boundaries-proposal.html)与[记忆、检查点与恢复边界研究提案](https://noemion.github.io/spec/memory-checkpoint-and-resumption-proposal.html)。

## 三个实现域不能合并

在同一命令命名空间下组织公开动作可以降低记忆负担，但不能让写入、独立读取和受限运行共享同一信任结论：

| 实现域 | 负责什么 | 当前限制 |
| --- | --- | --- |
| producer | 消费受控来源、意义决定、精确规范和 Profile；确定性形成来源保留的 Endem，以及未来物理 Profile 确定后的 closure 与发布派生 | 不替权威选择意义，不持有私钥或实时能力；发布 Profile 未定义时不执行裁剪派生 |
| inspector | 独立读取实际 Endem 字节，在精确规则、视图和预算下输出只读视图、差异与诊断 | closure、evidence 与发布制品等待物理格式；不复用形成侧解析实现，不产生生产检查通过结论 |
| runner | 重新核对精确运行输入、外部陈述、政策、环境、能力与预算；建立 contract 并控制一次会话 | 不信任路径名、缓存结论或 inspector 输出，不写制品，也不替具名权威接受结果 |

组件页分别给出 [producer](../components/producer.html)、[inspector](../components/inspector.html)与 [runner](../components/runner.html) 的完整输入、输出、失败责任和停止条件。当前没有三个组件、`endem` CLI、closure 物理格式、contract 运行时或 evidence 物理格式。
...
| 遇到的问题 | 继续阅读 | 先保留的边界 |
| --- | --- | --- |
| 想查一次操作由谁提出、谁执行、谁授权和谁作最终决定 | [Agent 系统边界图](../architecture/agent-system-boundaries.html) | 分开规范要求、研究提案和未来实现证据 |
...
**正在研究：**closure 与最终发布制品的物理 Profile、Agent 身份与委托、能力协商、隔离、跨会话恢复、并行提交、模型评测和训练更新仍需独立证据与决定。

**限制条件：**当前没有组件实现、运行时或互操作实验。producer、inspector、runner、closure、contract 与 evidence 已按普通词规则接受，但仍只是设计职责；Noemion 与 Endem 两个自造名称尚缺发行前的人类读音证据。

约束未来规范或实现前，使用[规范与查询](../specifications/)定位权威源，并分别说明支持案例、失败案例、适用范围和当前仍未覆盖的条件。
