...
4. **恢复必须重新验证。** 重新绑定精确 Endem 或 Synem、实际主体、政策、授权、端点、协议、schema、模型、环境、能力、预算、已消费决定和未知副作用。
...
| **Theor** | 任意不可信 Endem、Synem 或 Iknem 字节 | 有界只读视图、差异、引用、大小和跟踪信息 | 畸形输入、未知关键结构、资源超限、无法比较；不能产生 Ktisor 内部检查通过引用 |
...
“系统收到反馈”也不等于“系统已经学习”，更不等于“新模型可以发布”。会话历史、检索、提示变化、反馈记录、训练数据、权重更新、评测、部署和回滚必须分开。训练或微调形成新的外部派生模型身份；反馈只有另行固定显示、来源、资格、用途和权威后才可能成为训练输入。完整边界见[模型训练与更新边界研究提案](https://github.com/Noemion/noemion.github.io/blob/main/spec/model-training-and-update-boundaries-proposal.md)；它当前非规范，不创建训练 CORE、模型制品、训练平台或新专名。
