...
- 状态：Accepted；决定一的五字段语义与根 `aim` 已由 ADR-0009 取代
- 日期：2026-07-12
...
> 历史状态：本 ADR 继续决定 Endem/Weave/Frame/Witness 名称、状态机、单一 CLI、组件与权限拓扑。下文 `say/aim/must/done/open` 和根 `aim` 规则只记录当时决定，现行语义以 ADR-0009 的 `say/mean/case/when/open` 与一个根 `case` 为准。
...
- A2A 1.0 的 Task 与 Artifact 只用于跨系统任务交换，不定义 Endem、Frame、Witness 或最终验收；链接固定到 v1.0.1 文档快照：https://a2a-protocol.org/v1.0.1/specification/
...
- 名称、三个状态、`.endem`、八个动作与三组件拓扑继续有效；五组语义与根节点规则以 ADR-0009 为准。
