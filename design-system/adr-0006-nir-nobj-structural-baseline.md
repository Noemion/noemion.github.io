...
`magic`、`format_major`、`format_minor`、`header_size`、`object_kind`、`encoding_profile`、`flags`、`required_features`、Section Directory 的偏移/数量/表项大小、Load Directory 的偏移/数量/表项大小、字符串表索引、完整性 Section 索引和对象字节摘要。
...
| 记录 | 最低逻辑字段 | 主要不变量 |
| --- | --- | --- |
| Symbol | name ref、kind、binding、visibility、definition state、section/record ref、type ref、version ref、size or arity、origin ref、stable-key digest | 名称不是身份；定义状态、记录类别和类型必须一致；稳定键不能因 Section 重排改变。 |
...
`noesis`、`morphe`、`theoria` 和 `synthesis` 的手册必须使用同一逻辑字段、Section 家族和失败层次。NIR 文本语法、魔数、最终字段宽度、Section 编号、Symbol/Relocation 数值枚举、压缩算法、摘要算法、文件扩展名和签名封装仍需规范样本、威胁模型、跨实现原型与后续 ADR 冻结。
