...
证据时间：2026-07-13T19:51:48+08:00
工程责任：首次发行负责人
法律责任：目标法域的合格知识产权专业人员
状态：开发阶段工程初筛；ADR-0031 已接受一次性迁移；不是法律意见
...
- 当前工程结论不再是统一的中等风险。Endem 保持中等风险；旧名称的高风险已经通过直接替换处理；Drasor、drase 与 Iknem 只通过开发阶段初筛，首次正式发行仍由法律和实际包名取得门禁阻断。
...
Synem Dromen Iknem
Poiet Theor Drasor
poie elenk pleko tasse sphra theor drase peira
```
...
| `Poiet` | 本轮没有找到可信度足以进入相邻冲突表的精确 AI 或开发工具产品 | 搜索为空不是权利或未来可用证明 | 未证实低风险；继续复查 |
...
| `Synem / Dromen / Theor / Poiet` | 保持现行设计名并继续监测 | 正式发行前更新精确、近音、产品和商标检索，不得称为全网唯一 |
...
旧名称已经触发第一项停止条件并退出当前接口。[ADR-0031](../architecture/adr-0031-release-name-collision-gate.html)记录了候选比较、Iknem 与 Drasor/drase 的采用决定，以及不保留兼容入口的一次性迁移规则。

## 词源边界

语言学后缀 `-eme` 只帮助解释“最小区别单位”的构词思路。古希腊词根也只帮助记忆。`rhem`、`semion`、`skena`、`telis`、`krin`、`apor`、Synem、Dromen、Iknem、Poiet、Theor 与 Drasor 的含义始终由 ADR、数据结构、不变量和测试定义；词源不能替代工程规范。
