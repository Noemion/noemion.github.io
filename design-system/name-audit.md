...
现有名称检索只覆盖书写、发现和权利风险，没有进行受控人类发音实验。本节记录可从拼写和权威词典直接发现的首轮风险，不把观察升级为最终读音或改名决定。
...
| `Drasor` / `drase` | 采用为受限执行域与对应动作的设计阶段名称 | 正式发行前重查工程生态、口头区分并完成专业权利复核；取得实际分发坐标 |
| `Iknem` | 采用为有范围证据记录的设计阶段名称，标准为 IKN-CORE | 正式发行前重查工程生态、完成高风险读音复核和专业权利复核；不得把名称当作真实性或充分性声明 |
| `Ktisor` / `ktise` | 采用为确定性制作组件与对应动作的设计阶段名称 | 正式发行前重查大小写、近音、AI 产品、开发工具、命令、包坐标和高风险读音；取得实际分发坐标 |
| `TEXT-IDENTIFIER-CORE` | 采用为跨制品文本与标识符标准 ID，条款使用 `TEXT-*` | 保持它与 `.txt`、原始来源文件和未来组件名分离；规范职责变化时重审名称 |
| `Synem / Dromen / Theor` | 保持现行设计名并继续监测 | 正式发行前更新精确、近音、产品、商标和口头区分审查，不得称为全网唯一或已验证可发音 |
