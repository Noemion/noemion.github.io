...
[`telis` 发行词候选提案](https://noemion.github.io/spec/telis-release-terms-proposal.html)把职责透明、词义、读音、成对混淆、工程误导和反例作为六项桌面门禁。`make/keep` 会混淆 GNU Make 与目标方向，`attain/sustain` 的共同韵尾过强，`become/remain` 会暗示现行规范没有要求的前态转变，`achieve/maintain` 仍容易暗示行动者成功。`reach/maintain` 因而只取得“进入人类测试”的资格。
...
| `ktise` | [PyPI JSON API](https://pypi.org/pypi/ktise/json)、[npm Registry](https://registry.npmjs.org/ktise)、[crates.io API](https://crates.io/api/v1/crates/ktise) | 2026-07-16 分别为 HTTP 404、HTTP 404、HTTP 403 | 前两项只表示查询时没有精确项目；第三项证据不可用，不能声称 crate 不存在 |
| `ktisor` | [PyPI JSON API](https://pypi.org/pypi/ktisor/json)、[npm Registry](https://registry.npmjs.org/ktisor)、[crates.io API](https://crates.io/api/v1/crates/ktisor) | 2026-07-16 分别为 HTTP 404、HTTP 404、HTTP 403 | 前两项只表示查询时没有精确项目；第三项证据不可用，不能声称 crate 不存在 |
| `text-identifier-core` | [PyPI JSON API](https://pypi.org/pypi/text-identifier-core/json)、[npm Registry](https://registry.npmjs.org/text-identifier-core)、[crates.io API](https://crates.io/api/v1/crates/text-identifier-core) | 2026-07-16 分别为 HTTP 404、HTTP 404、HTTP 403 | 前两项只表示查询时没有精确项目；第三项证据不可用。标准 ID 也不等于未来软件包名 |
...
| `TEXT-IDENTIFIER-CORE` | GitHub 精确仓库名为零；PyPI 与 npm 没有精确项目；crates.io 本次查询不可用；GNU 软件目录没有精确文本 | 完整列出两个治理对象，不伪装成文件格式、组件、软件包或工程品牌 | 采用为规范 ID；职责变化时重新审查，不把空搜索写成名称保留 |
...
| `Ktisor` / `ktise` | 没有发现精确相邻产品或 GitHub 仓库名；PyPI 与 npm 没有精确项目，crates.io 本次查询不可用；组件与动作关系清楚 | 现行设计标识；专名必要性、读音、分发坐标和权利仍待发行复核 |
...
| `TEXT-IDENTIFIER-CORE` | 采用为跨制品文本与标识符标准 ID，条款使用 `TEXT-*` | 保持它与 `.txt`、原始来源文件、字段类型、未来组件名和软件包名分离；口头先说“文本与标识符核心规范”，职责变化时重审名称 |
...
| `rhem / semion / skena / telis / krin / apor / phain` | 保持现行设计字段；非规范提案建议七个直白职责名 | 先验证来源/投影、事态/观察、方向/判据、未决意义/观察不足四组边界；迁移必须显式处理新规范与 Profile 身份 |
...
旧名称已经触发第一项停止条件并退出当前接口。[ADR-0031](../architecture/adr-0031-release-name-collision-gate.html)记录 Iknem 与 Drasor/drase 的设计标识采用决定；[ADR-0032](../architecture/adr-0032-deterministic-maker-name-collision.html)记录 POIE 冲突、Ktisor/ktise 的设计标识采用决定、形成责任边界和动作成熟度差异；[ADR-0033](../architecture/adr-0033-text-identifier-specification-name.html)记录文本与标识符标准 ID 的可读性问题、候选冲突和一次性迁移；[ADR-0034](../architecture/adr-0034-pronunciation-and-oral-distinction.html)记录读音与口头区分门禁。前三项迁移都不保留兼容入口；第四项不创建读音别名。后续[发行术语提案](https://noemion.github.io/spec/release-terminology-simplification-proposal.html)、[语义面和观察词提案](https://noemion.github.io/spec/semantic-facet-terminology-proposal.html)与[生命周期和结果词提案](https://noemion.github.io/spec/lifecycle-and-result-terminology-proposal.html)重新审查现行标识及对象边界，但在人类验证和新 ADR 前不撤销既有决定。
