...
修改 `.endem`、`rhem/semion/skena/telis/krin/apor`、`phain`、事态投影、引用、内容身份、规范排序、封装布局或证据排布时，先读取当前 Endem、Synem 和 Tekmor 规范，再读取 [`ADR-0010`](../architecture/adr-0010-native-lexicon.html) 与相关活跃 ADR。架构决策只在 `architecture/` 维护；本目录不保存历史 ADR 副本，也不得重新引入旧词或兼容接口。
...
修改否定极性、负观察、空日志、查询未命中、观察故障或封闭范围时，必须读取 ADR-0017、Endem 与 Tekmor 规范以及 Praxor 页面。不得删除关系节点、另造否定谓词，或把未找到记录直接写成负事实。

修改“所有”“任一”、数量条件、成员范围、空集合或量化聚合时，必须读取 ADR-0018、Endem 与 Tekmor 规范以及 Praxor 页面。页面必须先解释集合怎样冻结，再解释量词；不得把搜索结果冒充全集、把重复事件当不同成员或用默认空真产生满足。

修改分数、指标、单位、窗口、均值、分位数、固定基准、推广结论、不确定度或阈值时，必须读取 ADR-0019、Endem 与 Tekmor 规范以及 Praxor 页面。页面必须先解释究竟测量什么和适用于谁，再解释程序与阈值；不得让排行榜、显示舍入值、点估计或模型置信度代替测量契约。

修改“并且”“或者”、复合条件、多个判据、短路或 Endem/Synem 拆分时，必须读取 ADR-0020、Endem、Synem 与 Tekmor 规范以及 Praxor 页面。页面必须先解释这是一个不可分终态还是多个独立目标，再解释 `all_of / any_of`；不得把未求值写成 `agno`，也不得让布尔表达式隐藏故障。
