...
- 指南和手册不使用普通页面的大面积渐变制品卡；手册引言使用折页，章节标题使用梯形页签，动态索引使用同一页签节奏。Endem 手册按 `form/lint/compose/inspect/run` 的职责色区分主题，但不改变折页结构、目录与阅读列。完整规则见 [`geometric-layouts.md`](geometric-layouts.md)。
