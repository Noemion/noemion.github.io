...
- Noemion 只作为项目与新领域的品牌，不进入格式名、对象类型、命令、ABI、魔数或组件前缀。Endem 是最小、独立有效且可验证的期望终态单元，正式扩展名为 `.endem`；唯一公开命令行为 `endem`，动作固定为 `form`、`check`、`bind`、`pack`、`seal`、`see`、`run` 与 `test`。不得建立并列工具品牌、旧命令别名、重定向或兼容入口。
