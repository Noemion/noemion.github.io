...
- Noemion 只作为项目与新领域的品牌，不进入格式名、对象类型、命令、ABI、魔数或组件前缀。Endem 是最小、独立有效且可验证的期望终态单元，正式扩展名为 `.endem`；每个 Endem 使用 `rhem/semion/skena/telis/krin/apor` 六个语义面并只有一个根 `skena`。唯一公开命令行为 `endem`，动作固定为 `poie`、`elenk`、`pleko`、`tasse`、`sphra`、`theor`、`praxe` 与 `peira`。不得建立旧字段或旧命令兼容入口。
