...
- 全站统一采用 Noemion 设计系统：共享顶部悬停展开导航、四列发现页脚、可持久化的 `Light / Dark / System` 主题选择、约 1200px 居中画布、编辑式排版、横向内容条目与快速微交互；不同页面角色分别使用列表、文章、工具项目和文档范式，不套用同一内容模板。视觉识别使用自有语义频谱、对象记录、证据状态、内容和产品状态语言。
- 全部正式页面必须按 `design-system/geometric-layouts.md` 继承所属模块的几何母题；同一模块保持一致，目录、专题、工具项目和手册只按页面角色调整面积与密度。三角形、梯形、平行四边形、折页和缺口形状必须对应真实内容关系，不能为了新奇堆叠装饰，也不能裁切正文、交互命中区或规范性信息。
...
- 所有正式文案、组件、对象、工具、目录和文件命名统一遵循 `design-system/language-and-naming.md` 与命名 ADR；中文使用主动语态和现在时，明确输入、动作、结果与失败，禁止无主语被动句、名词堆叠和管理套话。旧名称与旧路由不保留兼容入口。
- Noemion 是项目与对象体系的品牌，不是所有工具命令的强制前缀。工具可以采用独立的哲学相关名称，但必须同时提供清楚的中文职责副标题，并通过命名 ADR 说明语义、工程价值、冲突检查和迁移范围；不得为了系列感给无关工具强造晦涩名称，也不得让命令名变化影响 NIR/NOBJ 的格式身份。
- 面向读者的信息架构必须使用中文技术网站常见且职责明确的栏目名称：整体结构与关键决策写“架构设计”或“架构决策”，任务型内容写“指南”，按术语、对象或工具查找的内容写“参考指南”或“参考资料”，单个工具的连续专题写“使用手册”。不得用孤立的“架构”“文档”“参考”作为无法判断内容类型的页面主标题或导航名称；项目专有英文名和规范标识不受此限制。
...
- 架构组件、工具职责和页面组件都必须说明存在必要性、自身价值、上游输入或触发者、下游消费者、与相邻组件的边界，以及可以省略、合并或停止建设的条件；不得仅为架构图对称、目录完整或视觉填充保留无人消费的产物、重复工具或无信息价值的界面元素。
- Agent Harness 是 Fulfillment Runtime 外侧的控制平面：负责上下文装配、类型化能力、策略检查、观察反馈、预算与人工升级，不属于确定性可信核心。模型只能提出计划和能力参数；不得直接生成 NIR/NOBJ、修改签名对象、自行扩大权限或宣告验收通过。涉及智能体工程的页面与工具职责必须遵循 `design-system/adr-0002-agent-harness-boundary.md`。
...
- 工具页“继续阅读”章节只放置可点击资源入口，不得在卡片之后追加“尚未冻结的接口包括……”等孤立尾段。接口状态、开发顺序或候选契约必须放在“当前状态”或“它不会做什么”的明确上下文中，且不能与全站未发布声明重复。
