...
- 可读性优先于填满画布：1200px 是页面画布而不是段落宽度，技术长文和 Endem 应用说明的普通正文保持约 700–760px 阅读列与 17–18px/约 1.75–1.8 行高；桌面可使用约 800px 主阅读列和约 380px 粘性信息栏，但信息栏必须从 Hero 起承担目录、状态、关键制品、输入输出或后续入口等真实职责，内容不足时回到单列。具体尺度与验收读取 `design-system/readability.md`。
...
- Noemion 只作为项目与新领域的品牌，不进入格式名、对象类型、命令、ABI、魔数或组件前缀。Endem 是最小、独立有效且可验证的期望终态单元，正式扩展名为 `.endem`；每个 Endem 使用 `say/mean/case/when/open` 命题投影语义核并只有一个根 `case`。唯一公开命令行为 `endem`，动作固定为 `form`、`check`、`bind`、`pack`、`seal`、`see`、`run` 与 `test`。不得建立旧字段或旧命令兼容入口。
- 语言哲学相关设计严格遵守 ADR-0009 的工程边界：来源记号、意义投影、可能事态与满足观察必须分层；命题结构必须表达对象之间的关系和位置；逻辑形式由结构显示，不能由载荷自我宣称；`no-sense`、语义 `open` 与运行期 `unknown` 不得合并。公开页面必须同时说明《逻辑哲学论》不能直接推出软件规范，也不得声称格式编码了不可说之物。
- 正式术语必须同时保持制品与权限边界：Weave 是已解析并可封装的组合闭包，Frame 是已装载运行态，Witness 是带声明范围的证据。`see` 背后使用与生产写入路径独立实现、只读且无写入/密钥/运行权限的 Reader；`run` 背后使用独立 Runner 与最小能力域。统一 CLI 不得被描述成共享实现或共享权限。
...
- Runner 外侧的控制平面负责上下文装配、类型化能力、策略检查、观察反馈、预算与人工升级，不属于确定性 Core。模型只能提出不可信来源候选、计划和能力参数；不得决定 Endem/Weave 规范字节、修改已封装或已签名制品、自行扩大权限、删除 `open` 项或宣告验收通过。涉及智能体工程的页面必须同时说明 Runner、控制平面、Witness 与人工验收的责任边界。
