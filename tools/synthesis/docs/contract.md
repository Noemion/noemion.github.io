---
layout: "manual"
title: "职责与契约 · synthesis 使用手册 · Noemion"
page_role: "docs-topic"
footer_text: "Noemion · synthesis 使用手册"
permalink: "/tools/synthesis/docs/contract.html"
manual_id: "synthesis"
manual_group: "start"
manual_order: 1
nav_title: "职责与契约"
hero_title: "职责与契约"
hero_description: "说明 synthesis 负责什么、拒绝什么，以及哪些边界可以依赖。"
summary: "说明 synthesis 负责什么、拒绝什么，以及哪些边界可以依赖。"
badges: ["synthesis", "Phase 4 / Phase 5"]
---

## 问题与目标

链接阶段要把多个分别有效的对象组合为一个整体有效的对象。局部有效并不推出组合后仍有效：两个对象可能导出同一符号、提出互斥约束、要求不同 ABI，或在合并后扩大权限。因此 `synthesis` 的目标不是“尽量生成输出”，而是在确定性规则足以证明组合成立时生成输出，否则以结构化证据拒绝。

## 职责

- 验证并合并兼容 Section 与多层 NIR 图。
- 执行 local/export/import、版本、可见性、显式缺失策略与归档成员解析。
...
| 闭包完整 | 所有 required import、必需依赖、权限要求和披露要求均可解析。 | 无输出，生成冲突报告。 |
