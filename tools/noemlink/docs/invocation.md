...
CLI 只是链接契约的一种前端。无论未来由命令行、构建系统还是库接口调用，都必须解析为同一个显式链接请求：有序输入声明、内容身份、Link 配置、依赖锁定、输出意图和诊断策略。环境变量、当前目录或网络状态不得暗中改变 Deterministic Profile结果。

| 意图 | 语义 | 必要输出 |
| --- | --- | --- |
| 最终链接 | 形成供装载器验证的封闭产物候选。 | Linked Object、链接映射、验证摘要。 |
...
```text
noemlink main.nobj libdomain.noema -o task.noemx
noemlink -shared skill/*.nobj --lock deps.lock -o skill.dev.hobj
noemlink -r part-a.nobj part-b.nobj -o combined.nobj
```
