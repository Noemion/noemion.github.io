...
当前解释从“更新依赖并判断能否发布”开始。锁文件、部署响应、健康探针和模型说明分别对应有限主张；随后按固定对象、保留观察与变换、有效性评估、覆盖计算、满足判断和权威决定给出连续顺序。IKN-CORE 的九条现行责任仍可逐项定位，物理格式与实现状态也保持明确。
...
## 规范模块平行四边形方向复核 · 2026-07-15

规范模块的引言以大平行四边形表示条款、编码和版本沿确定方向映射，以章节标题前的小平行四边形标记后续阅读单元。此前只按引言装饰的前后叠放关系判断大小，误把黑色命题面归为小形，导致它沿用左下贴齐的方向；但从页面中的实际面积与职责看，命题面和引言背景都属于大形。

当前共享规则让引言背景和黑色命题面都采用左上贴齐、左下收进的前向轮廓；章节标题前的短标继续采用左下贴齐、左上收进的反向轮廓。大形与小形形成稳定的两种朝向，规范目录页和全部规范专题页通过同一组模块变量继承，不为外部协议适配页面建立特例。形状仍不裁切标题、徽标、链接或正文，也不改变规范含义、成熟度和实现状态。
