...
| P0-W3 规范向量 | 6 个语义 JSON 向量、6 个 END-P0 结构字节、7 个 END-P1 完整载荷字节 | 已执行语义、结构与完整载荷三层结果；END-P1 覆盖最小接受、非空 apor 接受和 5 类确定拒绝 | 仍缺截断、乱序、非最短 CBOR 与更多资源边界向量 |
...
| P1-W1 来源与安全核心 | 2 个 END-SRCM 来源样例、本地 Rust Poiet/Elenk/Theor/CLI、7 个 END-P1 字节向量与 3,372 个有界变异样本 | 最小清单与非空 apor 清单都映射为规范对象；两条读取路径不共享解析代码 | 远端仓库、公开 CI、覆盖引导模糊、故障注入和跨平台二进制复现 |
