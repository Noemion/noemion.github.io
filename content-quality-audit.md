...
以下清单记录初次逐页审计，并在后续新增 ADR-0022 至 ADR-0029、Dromen、诊断、适配、精确身份、文本以及授权规范页时继续按同一标准复核。当前共有 52 个 HTML 正文源；Markdown 生成页面另由路由测试和构建产物审计覆盖。
...
本轮逐一复核 `sitemap.md` 登记的 65 条正式路由，包括 52 个 HTML 正文源和 13 个由 Markdown 生成的页面。复核重点不是统一文风，而是让每种页面先完成自己的读者任务：门户给出项目定义，目录给出选择依据，专题给出结论与边界，应用给出状态与输入输出，手册给出连续操作逻辑。
...
| 架构与 ADR（26） | `/architecture/index.html`、`/architecture/endem-lifecycle.html`、`/architecture/decisions.html`、`/architecture/adr-0008-endem-system.html` 至 `/architecture/adr-0029-authority-and-authorization-decisions.html`、`/architecture/open-questions.html` | 生命周期先解释每阶段回答什么；历史 ADR 首屏标明怎样阅读和哪些名称已经失效；ADR-0011 至 0029 依次固定容器、候选语言、内容 Profile、来源清单、结果域、时间、否定、量化、测量、复合判断、组合、证据、内容分层、会话、诊断、适配、身份、文本以及授权边界。 |
...
外部协议适配、精确身份、文本边界、权威授权规范及 ADR-0026 至 ADR-0029 与更新后的规范入口继续使用同一组视口验收；它们必须保持表格局部滚动、移动目录可达、标题自然换行和零控制台错误。
...
| 工作包 | 已形成证据 | 当前结论 | 剩余缺口 |
| --- | --- | --- | --- |
| P0-W1 权威规范 | 十一份核心与格式规范源、149 个唯一条款 ID、END-P0/END-P1 与 `spec/registry.json` | 对象与诊断、适配、身份、文本、权威授权边界分层登记；实现、线格式和证据状态独立 | 代码阶段前继续用真实自然语言案例与反例修订；独立实现验证等待用户开启代码阶段 |
| P0-W2 威胁与限制 | 对象及诊断、适配、身份、文本与授权威胁模型、`spec/diagnostic-catalog.md` 和 `spec/profiles/end-p0.json` | 已建立 98 类威胁、8 项 END-P0 有限上限和跨对象诊断目录 | 仍需边界规模、最坏复杂度、目标平台、密码、Unicode 与授权实验证据 |
| P0-W3 场景与规范向量 | 153 个非规范自然语言场景、7 个 Endem 语义 JSON 向量、七组各 12 个专题向量、20 个 Dromen、18 个 Tekmor、20 个诊断、各 24 个适配、身份、文本与授权向量、6 个 END-P0 与 14 个 END-P1 字节 | 场景覆盖语义、判断、组合、闭包、会话、证据和横切边界；所有提案矩阵均执行允许与拒绝分类 | 场景和提案向量不是组件证据；仍缺未来物理编码、协议、签名、Unicode 与授权 Profile 及平台证据 |
