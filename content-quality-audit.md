...
首页标题进一步采用同名仓库的时代宣言，把主张写成“人工智能时代，每个人都应该能编译自己的意图”。标题只负责建立方向和传播张力；封面摘要改用“以自然语言表达意图”“完整性可验证的目标制品”“任何改动都可被识别”等简洁学术用语解释价值。摘要不再把“长期保存”当作与 AI Skill、提示词文件或普通存储的核心差异，也不再用没有说明检查者和检查对象的“独立检查”代替安全含义。研究状态由相邻状态区承担，自然语言生成代码和尚无组件等反例进入后续正文。
