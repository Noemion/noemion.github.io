...
新增几何可读性检查：不能只验证没有横向溢出，还要检查段落实际行宽、字号、行高和粘性信息栏是否承担真实职责。当前审计发现部分普通专题的横向行项目文字可达到约 1080px，工具项目正文仍为 16px，且工具信息栏没有从 Hero 起形成连续列线；这些属于下一轮实现必须消除的可读性缺口。目标基线为普通正文约 700–760px、桌面 17–18px/约 1.75–1.8 行高，长篇技术页与工具页可采用约 800px 主阅读列和约 380px 粘性信息栏。
