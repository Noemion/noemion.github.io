...
Noemion 已经接受以 Endem 为核心制品的工程词汇、单一应用拓扑和 END-FMT 实验性编码。首个本地 Rust 候选已实现四个动作并通过权威向量、独立读取、有界变异和对象字节复现，但实现仓库、安装包、稳定 ABI、跨平台结果和正式版本尚未公开。Endem 当前仍是中等风险的开发候选名：精确包名未登记，但 GitHub 已存在第三方同名对象，正式商标和法律门禁尚未完成。页面与本地测试存在不等于软件已经适合生产，也不证明性能、研究结论、知识产权或标准化状态。
...
本轮逐一复核 `sitemap.md` 登记的 44 条正式 HTML 路由，包括 31 个 HTML 正文源和 13 个由 Markdown 生成的页面。复核重点不是统一文风，而是让每种页面先完成自己的读者任务：门户给出项目定义，目录给出选择依据，专题给出结论与边界，应用给出状态与输入输出，手册给出连续操作逻辑。
...
| 架构与 ADR（11） | `/architecture/index.html`、`/architecture/endem-lifecycle.html`、`/architecture/decisions.html`、`/architecture/adr-0008-endem-system.html`、`/architecture/adr-0009-propositional-kernel.html`、`/architecture/adr-0010-native-lexicon.html`、`/architecture/adr-0011-endem-container.html`、`/architecture/adr-0012-rust-core-language.html`、`/architecture/adr-0013-end-p1-payload.html`、`/architecture/adr-0014-source-manifest.html`、`/architecture/open-questions.html` | 生命周期先解释每阶段回答什么；历史 ADR 首屏标明怎样阅读和哪些名称已经失效；ADR-0011 至 0014 分别固定容器、语言、实现 Profile 和来源清单；开放问题改用直白问题与发布条件。 |
...
| MCP 2025-11-25 稳定版与 2026-07-28 发布候选 | 当前稳定版仍是 2025-11-25；候选版改为无状态核心、扩展框架并强化授权，且明确包含破坏性变化 | 版本化外缘适配、最小能力、受众校验、拒绝和观察记录 | 候选版在正式发布与重验前不得进入基线；远端结果不能直接成为验收事实 | 2026-07-28 正式版发布，或授权、传输、工具结构变化 |
...
| 工作包 | 已形成证据 | 当前结论 | 剩余缺口 |
| --- | --- | --- | --- |
| P0-W1 权威规范 | `spec/endem-core.md`、`spec/endem-format.md`、`spec/endem-source-manifest.md`、43 个唯一条款 ID、END-P0/END-P1 与 `spec/registry.json` | 语义、结构、首个载荷 Profile 和来源清单分层登记；实现和稳定 ABI 状态独立 | 继续用真实语料与两个独立实现反驳和修订 |
...
| P0-W3 规范向量 | 6 个语义 JSON 向量、6 个 END-P0 结构字节、13 个 END-P1 字节 | END-P1 覆盖两个接受、5 类语义拒绝、截断、目录乱序、非最短 CBOR 和三类载荷资源上限 | 仍缺累计分配、偏移算术和组合引用边界向量 |
| P0-W4 语言与构建 | C/Rust 双原型、6 个规范向量、144 个差分变异、Sanitizer、10,000 次 C libFuzzer、重复构建和 `results.json` | ADR-0012 已为 Poiet 与生产读取核心选择 Rust 1.97.0；后续 P1 候选已补 Rust fuzz 目标 | 仍需跨平台复现与更长的多环境 campaign |
| P1-W1 来源与安全核心 | 2 个 END-SRCM 来源样例、本地 Rust Poiet/Elenk/Theor/CLI、13 个 END-P1 字节向量、3,372 个有界变异和每条读取路径 100,000 次 fuzz | 两条路径对分层错误一致；Poiet 编码前执行预算；基于规范种子的覆盖引导运行未发现崩溃或语义绕过结构拒绝 | 远端仓库、公开 CI、长时 fuzz、故障注入和跨平台二进制复现 |

`tests/spec_contract_test.py` 检查规范版本、条款唯一性、成熟度、威胁映射和向量登记；`tests/source_manifest_test.py` 把来源清单映射到语义接受向量；`tests/semantic_vector_test.py` 执行 JSON 语义外壳；`tests/wire_vector_test.py` 读取 END-P0 结构字节；`tests/p1_payload_test.py` 确定性编码并解码 END-P1。它们证明当前公开规范资料与这些案例一致，不证明未来稳定发行没有其他缺陷。
