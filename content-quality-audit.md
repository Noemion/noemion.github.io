...
| P0-W3 规范向量 | 5 个语义向量、6 个十六进制字节向量、`tests/wire_vector_test.py` | 已实际解码 1 个结构接受与 5 个确定拒绝案例，并保持结构/语义接受分离 | 仍缺完整字段载荷、截断、乱序、非最短 CBOR 与资源边界向量 |
...
`tests/spec_contract_test.py` 检查规范版本、条款唯一性、成熟度、威胁映射和向量登记；`tests/wire_vector_test.py` 独立读取头部、目录、范围与受限 CBOR，并比较稳定拒绝结果。它们证明当前规范资料自洽，不证明未来生产实现符合规范。
