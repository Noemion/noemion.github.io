...
Noemion 已经接受以 Endem 为核心制品的职责、单一应用拓扑和 END-FMT 实验性编码，但尚未进入组件代码开发阶段。具体发行拼写与读音尚未通过人类验证；后续[发行术语去专名化研究提案](https://noemion.github.io/spec/release-terminology-simplification-proposal.html)又建议先证明专名必要性，并把 Endem 之外的现行对象称呼、信任角色和公开动作同直白职责候选比较。[语义面与观察词提案](https://noemion.github.io/spec/semantic-facet-terminology-proposal.html)进一步保留七项职责，却建议以直白字段名替换 `rhem/semion/skena/telis/krin/apor/phain` 进入验证。[生命周期与结果词提案](https://noemion.github.io/spec/lifecycle-and-result-terminology-proposal.html)又发现 `attested` 把外部签名和证明关系误写成内容状态，并把对象边界修正与 `nascent/coherent/agno/aseme/interrupted` 的发行名称验证分开。三项建议都尚未成为 ADR 或迁移决定。当前没有 Ktisor、Theor、Drasor、`endem` CLI、安全核心、安装包或实现 CI；Python 检查器只核对规范、登记和向量之间的一致性。Endem 当前仍是中等风险的候选名：精确包名未登记，但 GitHub 已存在第三方同名对象，正式商标、法律和口头区分门禁尚未完成。页面与资料测试存在不等于软件已经实现，也不证明性能、研究结论、知识产权、读音可用性或标准化状态。

[能力发现、协商与调用提案](https://noemion.github.io/spec/capability-discovery-and-negotiation-proposal.html)继续检查 Agent 运行边界。它把能力声明、协议协商、授权决定、Dromen 会话上限、即时可调用性与调用事实分开，拒绝让动态工具列表扩写旧会话，也拒绝让端点可达、scope、签名或一次成功调用变成长期权限、`met` 或 `accepted`。这仍是尚未进入规范的研究，不创建 `CAP-CORE`、能力制品、组件或新专名。
...
[并行、推测执行与提交边界提案](https://noemion.github.io/spec/parallel-and-speculative-execution-proposal.html)继续补齐多 Agent 与并行工具调用的高风险缺口。它把分支准入、候选结果、提交选择、当前前提、已发生副作用和后验观察分开，要求所有分支共享同一 Dromen 上限，并拒绝让最快完成、模型评分、取消或外部 Task 状态越过授权与结果域。这仍是尚未进入规范的研究，不创建 `PAR-CORE`、事务制品、调度器、组件或新专名。
...
以下清单记录初次逐页审计，并在后续新增 ADR-0022 至 ADR-0036、Dromen、诊断、适配、精确身份、文本、授权、伴随关系、名称门禁、动作收敛和裁剪发布页面时继续按同一标准复核。当前共有 58 个 HTML 正文源；Markdown 生成页面另由路由测试和构建产物审计覆盖。
...
本轮逐一复核 `sitemap.md` 登记的 123 条正式路由，包括 58 个 HTML 正文源和 65 个由 Markdown 生成的页面。复核重点不是统一文风，而是让每种页面先完成自己的读者任务：门户给出项目定义，目录给出选择依据，专题给出结论与边界，应用给出状态与输入输出，手册给出连续操作逻辑；其中 51 个 `spec/*.md` 页面由同一正文源自动生成 HTML，不再把 GitHub Markdown 作为站内页面替代品。
...
本轮在 393×852、768×1024、1024×768 与 1440×900 视口复核首页和 Dromen 指南：页面均无横向溢出，Noemion 品牌与“进展”入口不重叠，移动端目录可以展开，没有坏图、偏离居中的独立图片或控制台错误。导航资料检查同时要求每个入口使用对应封面，并为 Dromen 配置独立封面资源。
...
当前页面以“周一获准、周三撤销”的同一 END-P1 形成制品进入，先区分不改变内容身份的授权、签名、证据与决定变化，再说明会建立新身份的语义字节修改和未来发布派生。权威名称、五层符合性报告、四项条款和外部机制分别进入实现检查表。页面现有六个章节、五张表、二十四行表体、一条生命周期流程、四个按问题入口和 2,509 个可见字符；“内容身份不变，不等于授权仍然有效”直接给出停止结论，“内容身份、授权伴随关系、外部前置条件、截止点评估”保持职责与读音可区分。
...
本轮只重构公开 ADR、相邻页面标签、导航、架构索引、规范入口、站点发现摘要、质量契约和审计记录，并把已漂移的 TUF 版本引用从 1.0.27 更新为 1.0.35。END-CORE、END-FMT、END-P1、四项条款、六个记录、字段、登记、场景、语义向量与身份规则均未改变，也没有创建伴随 Profile、权威目录、政策语言、决定事件、撤销分发、服务或组件。

## ADR-0031 设计标识与发行名称边界二次复核 · 2026-07-16

ADR-0031 此前用六个章节、三张表、十行表体、零条流程、零个按问题入口和 1,628 个可见字符记录两项旧名的冲突以及 Iknem、Drasor、<code>drase</code> 的替换决定。页面把“采用”与一次工程初筛并列，虽然末尾保留正式发行停止条件，首次阅读者仍容易把当前设计拼写误解为已经通过发行名称审查；包坐标、商标、读音、职责必要性和重新审查触发条件也没有形成同一条路径。
...
截至复核时，Iknem、Drasor、<code>drase</code> 的 PyPI 与 npm 精确入口均返回 404；crates.io 自动查询返回 403，不能形成名称不存在的结论。WIPO Global Brand Database 明确提醒其覆盖多个国际和国家或地区集合，但未命中不等于名称可用，还应查询目标知识产权局并在需要时咨询商标专业人员。现有读音指南也确认 Noemion 尚未执行真实人类发现与发行审查，TTS、ASR、一次正确听写和维护者自拟 IPA 均不能成为发行证据。

本轮只重构公开 ADR、导航、架构索引、站点发现摘要、质量契约和审计记录。Iknem、Drasor、<code>drase</code>、<code>IKN-CORE</code>、对象职责、动作边界、一次性迁移和不兼容策略均未改变；直白职责名仍是非规范比较候选。当前没有官方读音、分发坐标保留、商标结论、组件实现、别名、重定向、双写或兼容垫片。
...
项目背景入口此前先讲“新的软件边界”，随后列出 Endem、Synem、Dromen、Iknem 四个对象和四类外部系统责任。四个章节只有 1,121 个可见字符，没有责任表或按问题入口；首次阅读者需要先理解内部对象，才能恢复 Noemion 为什么与普通人用自然语言表达的目标有关。“如何推进”又把未来验证顺序放在项目目的旁边，削弱了使命、当前边界和学习路径之间的层次。
...
Endem 应用入口此前先解释应用目的和当前状态，随后按五个动作展开流程、输入输出与禁止事项。页面要求首次读者在建立实际任务之前记住 <code>ktise / elenk / pleko / theor / drase</code>，并用连续七项否定句结束主要说明。状态栏又由共享 CSS 依据“第二个章节”定位；只要章节顺序变化，错误内容就会取得粘性侧栏职责。旧页面共有 4,473 个源字符、六个章节、一张表、一条流程和四个继续阅读入口。
...
ADR-0011 此前以 END-P0 为中心介绍九个主题、三张表和一条容器流程。页面能够说明固定前导、目录、受限 CBOR 和资源上限，却没有先给开发者一条完整读取顺序，也没有在首屏区分 END-P0 结构实验、END-P1 来源保留形成制品与尚未定义的发布 Profile。六个空映射、Profile 接受、内容接受和最终发布因此容易被误读为同一条“格式有效”结论。

当前页面先限定 END-FMT 只负责不可信字节的安全分段与确定解码，再按“身份与总大小—目录算术—记录范围—Profile 能力—确定性载荷—字段与引用—内容与外部判断”组织读取路径。END-P0 明确只保留结构实验，END-P1 明确是包含原始自然语言且不可发布的形成与评审 Profile；未来发布版不能从 END-P1 直接删除 <code>rhem.content</code>，而必须重写来源引用、重新证明闭包、取得新身份并重新验证。页面收敛为六个章节、五张边界表、一条读取流程、四个开发入口和 2,716 个章节可见字符；新增内容用于说明停止位置和声明上限，不增加字段或格式能力。
...
当前页面把 END-P1 明确为不可发布的来源保留形成与评审 Profile。读取路径依次经过 END-FMT、Profile 2、六记录字段、排序引用、外部内容前提和未来发布派生；六个记录改为回答来源、意义、事态、方向、判据与待确认项。页面明确现行向量只执行语言标签和媒体类型的基本语法，没有固定完整登记有效性、首选值替换或规范化算法。页面从六个章节、一张表和 1,543 个章节可见字符，重构为六个章节、五张边界表、一条读取流程、四个开发入口和 1,941 个章节可见字符。
...
ADR-0016 此前按时间范围、端点、连续性政策、观测覆盖和外部资料依次解释抽象模型，但没有用一项实际目标串起形成、停止、观察和分类责任。页面仍把 <code>mene</code> 放在标题和状态徽标中，未说明该拼写已经未通过首次发行桌面审查；OpenTelemetry 的区间端点和 Gauge 采样语义也没有与项目半开区间明确对照。旧页面共有八个章节、三张表、没有任务流程和继续阅读入口，共 2,478 个可见字符。

当前页面用“部署完成后十分钟内保持健康，允许有限中断”贯穿目标方向、经过时长、违约预算、覆盖证据和四值结果。固定区间与经过时长、严格连续与预算连续分别说明责任和拒绝条件；采样、遥测缺口、Agent completed 和远端时间戳都不能替代连续覆盖。<code>mene</code> 保持为现行草案值，但页面明确其发行拼写已经未通过桌面审查；<code>reach / maintain</code> 仍需真实使用者朗读、听写和成组口头区分。页面现有六个章节、五张边界表、一条任务流程、四个开发入口和 2,593 个可见字符。

[RFC 3339](https://www.rfc-editor.org/info/rfc3339/)与[RFC 9557](https://www.rfc-editor.org/info/rfc9557/)提供互联网时刻和附加时区信息，不替项目决定规范形式、时间权威或区间政策。[GNU C Library 时钟说明](https://www.gnu.org/software/libc/manual/html_node/Getting-the-Time.html)明确单调时钟适合经过时长但不能表示绝对时间；[GNU Coreutils 相对日期说明](https://www.gnu.org/software/coreutils/manual/html_node/Relative-items-in-date-strings.html)披露夏令时和日历运算风险。[OpenTelemetry Metrics 数据模型](https://opentelemetry.io/docs/specs/otel/metrics/data-model/)把 Gauge 定义为采样值，并为聚合点使用不同端点规则；[MCP 2025-11-25 Tasks](https://modelcontextprotocol.io/specification/2025-11-25/basic/utilities/tasks)与[A2A 1.0](https://a2a-protocol.org/v1.0.0/specification/)只提供外部任务生命周期和时间戳。这些事实都必须经带来源的适配与本地评价，不能直接产生 Iknem 覆盖或持续目标满足。

本轮只重构公开 ADR、导航摘要、站点地图、质量契约和审计记录。END-CORE、END-TIM 三条条款、现行草案值、END-P1、Profile、登记、十二个 <code>mene</code> 向量和读音结论均未改变，也没有创建时间字段、计时器、监控器、求值器、Drasor、协议适配器或兼容入口。
...
当前页面用“匿名主体没有读取私有报告”贯穿目标关系、角色、观察范围、完整性与四值结果，并先区分能力的显式负观察与行为事件的缺席证据。否定继续引用同一谓词和角色，不创建 <code>not_*</code> 谓词或重复的回避方向；开放日志无匹配为 <code>agno</code>，观察契约失败为 <code>fault</code>，同构正反例为 <code>unmet</code>，只有有权威且覆盖充分的封闭观察范围才可支持 <code>met</code>。哲学启发缩为关系同一性和采用限制，不再挡在工程路径之前。页面现有六个章节、五张边界表、一条任务流程、四个开发入口和 2,550 个可见字符。
...
本轮只重构公开 ADR、导航摘要、站点地图、质量契约和审计记录。END-CORE、END-NEG 三条条款、END-P1 枚举、Profile、登记、规范字节与十二个否定向量均未改变，也没有创建封闭声明格式、日志收集器、策略引擎、求值器、Drasor、MCP 适配器或兼容入口。
...
本轮只重构公开 ADR、导航摘要、站点地图、质量契约和审计记录。END-CORE、END-QNT 三条条款、量词与成员模式草案值、END-P1、Profile、登记和十二个量化向量均未改变，也没有创建集合解析器、成员目录、Ktisor、Drasor、求值器、MCP 适配器、CLI 或兼容入口。
...
本轮只重构公开 ADR、导航摘要、站点地图、质量契约和审计记录。END-CORE、END-MSR 四条条款、聚合器与总体模式草案值、END-P1、Profile、登记和十二个测量向量均未改变，也没有创建单位词典、遥测采集器、基准运行器、统计引擎、Drasor、求值器、CLI 或兼容入口。
...
当前页面用接口可达、响应结构和 p95 延迟贯穿同一终态、关系叶、独立判据、四值叶结果、总体组合与检查覆盖。一个复合 Endem 和多个 Endem 加 Synem 继续按终态身份、方向、生命周期、权威和失败责任区分，不再由自然语言连接词机械决定。<code>quantifier=all</code> 明确表示“全部成员”，<code>combiner=all_of</code> 明确表示“全部条件”；代码与日志必须带域名，口头说明先使用中文。页面现有六个章节、五张边界表、一条任务流程、四个开发入口和 2,548 个可见字符。
...
本轮只重构公开 ADR、导航摘要、站点地图、质量契约和审计记录。END-CORE、END-CMP 四条条款、END-P1、Profile、登记、规范字节和十二个复合判据向量均未改变，也没有创建表达式语言、组合字段、Ktisor、Theor、Drasor、求值器、CLI 或兼容入口。
...
本轮只重构公开 ADR、导航摘要、站点地图、质量契约和审计记录。SYN-CORE、六项条款、END-CORE、END-FMT、END-P1、Profile、登记、威胁、场景、向量、名称与读音结论均未改变，也没有创建 Synem 格式、解析器、形成器、远程仓库、调度器、Ktisor、Theor、Drasor、CLI 或兼容入口。

## ADR-0022 证据范围与评估边界二次复核 · 2026-07-16

ADR-0022 此前按九个问题、记录种类、来源类别、结果域、九条规则和外部资料依次解释 Iknem。规范边界正确，但开发者需要跨九个章节恢复一项发布检查从观察到决定的完整路径；<code>valid / invalid</code> 与 <code>sufficient / insufficient</code> 也没有给出日志域限定和口头易混淆提示。旧页面共有九个章节、三张表、没有任务流程和继续阅读入口，共 2,892 个可见字符。
...
本轮只重构公开 ADR、导航摘要、站点地图、质量契约和审计记录。IKN-CORE、九项条款、Profile、登记、威胁、场景、十八个向量、物理格式与现行设计名称均未改变，也没有创建采集器、验证器、归并器、撤销分发、决定引擎、Theor、Drasor 或兼容入口。

## ADR-0023 内容标准分层二次复核 · 2026-07-16

ADR-0023 此前已经区分 END-CORE、END-P1 与 END-FMT，并按三种接受、扩展规则和 ELF 类比解释边界。页面的抽象分层正确，但没有用一份实际 END-P1 文件说明读取顺序，也没有说明 P0、含来源形成 Profile 与未来裁剪发布 Profile 是不同用途而不是连续信任等级。更重要的是，END-P1 单文件声明上限已经是 Profile 接受，三个字节向量仍返回 <code>semantic-accept</code>，与外部授权前置条件尚未核对的事实冲突。旧页面共有七个章节、两张表、一条抽象流程、没有继续阅读入口，共 1,996 个可见字符。

当前页面用一次 END-P1 读取贯穿格式与 Profile 固定、容器检查、封闭字段检查、外部授权核对、END-CORE 内容接受、目标判断与最终决定。P0 明确只提供结构实验，P1 保存原始自然语言并最高支持 Profile 接受，未来裁剪发布必须采用新 Profile 和新身份。三个 END-P1 接受向量及检查器结果已经改为 <code>profile-accept / external-prerequisites-not-evaluated</code>，不再暗示内容层已经通过。页面现有六个章节、五张边界表、一条开发流程、四个继续阅读入口和 3,768 个可见字符。
...
本轮修正公开 ADR、导航摘要、站点地图、质量契约、END-FMT 接受层次、END-P1 向量清单与资料检查器。END-CORE、END-P1 字段、Profile 身份、实验字节、资源上限和现行容器布局均未改变，也没有创建写入器、读取器、CLI、运行时、裁剪命令、稳定 ABI 或兼容入口。
...
ADR-0026 此前先给抽象结论，再分别说明版本变化、Drasor 调用、外部状态、映射与重试、安全和权威资料。边界方向正确，但开发者需要跨八个章节把版本、对端、能力、调用、状态、取消、重试和网络安全重新拼成一次协议调用；四行状态表也不足以说明调用停止条件。旧页面共有八个章节、一张表、四行表体、一条抽象流程、没有继续阅读入口，共 2,026 个可见字符。

当前页面用一次外部发布调用贯穿协议版本、对端身份、能力交集、本地调用、外部操作、映射损失、候选产物和本地判断。取消请求、取消确认、停止观察和副作用回滚明确成为不同事实；只读、幂等和去重重试分别列出所需证据，所有尝试继续共享原调用身份、Dromen 预算、截止点与取消状态。凭据受众、重定向、DNS 重绑定、多租户发现和资源上限进入同一安全包络。页面现有六个章节、五张边界表、二十四行表体、一条调用流程、四个开发入口和 3,562 个可见字符；新增内容用于说明真实接线和停止条件，不创建协议字段或实现假设。
...
本轮只重构公开 ADR、导航摘要、开发者规范入口、站点地图、质量契约和审计记录。ADP-CORE、十二项条款、登记、威胁、十八个场景、二十四个提案向量、机器码和结果域均未改变，也没有创建协议 Profile、适配器 API、网络服务、凭据代理、重试引擎、Webhook 服务、事件存储、Drasor 或兼容入口。
...
ADR-0032 此前用七个章节、四张表、十二行表体、没有开发流程和继续阅读入口，记录旧组件与动作的名称冲突、Ktisor/<code>ktise</code> 候选比较、包入口初筛和动作成熟度。页面把 Ktisor 写成“确定投影”，容易让开发者误以为形成器拥有意义决定权；又把 2026-07-13 的空搜索直接连到“采用”，与 ADR-0031 已明确的设计标识和发行名称分层不一致。旧页面共有 1,667 个可见字符。

当前页面先把信任角色、<code>endem</code> 子命令、规范诊断和未来分发作为不同发布面，再用从有权主体确认意义投影到 Theor 独立读取的流程说明形成责任。Ktisor 只消费已确认且具有精确语义授权绑定的意义投影，负责规范顺序、布局、闭包和字节；它不选择意义、不删除未决项、不持有模型、私钥或实时能力，也不宣告目标满足或最终接受。页面现有六个章节、五张边界表、二十五行表体、一条开发流程、四个继续阅读入口和 2,915 个可见字符。
...
本轮重构公开 ADR、相邻入口、导航摘要、站点地图、名称审计和质量契约。旧词族已退出且不恢复；Ktisor 与 <code>ktise</code> 仍只是现行设计标识，确定性形成、唯一规范写入、生产侧分层检查、Theor 独立读取、动作成熟度和不兼容迁移策略均未改变。当前没有组件实现、CLI、发行包、官方读音、包坐标保留、权利结论、别名、重定向、双写或兼容垫片。
...
本轮重构公开 ADR、导航摘要、站点地图、名称审计和质量契约。<code>TEXT-IDENTIFIER-CORE</code>、十二项 <code>TEXT-*</code> 条款、威胁、场景、二十四个提案向量、END-FMT、END-P1、ASCII 结构标识符、Unicode 文本范围、内容身份和授权边界均未改变。旧缩写不恢复；当前也没有新文件格式、Unicode Profile、文本处理器、模型输入网关、稳定 API、发行包、字母逐读官方发音、别名、重定向、双写或兼容垫片。
...
当前页面用一次依赖升级贯穿自然语言来源、END-P1 形成版、语义复核、发布 Profile、确定性派生、新身份、外部陈述和发布前验证。删除原文、重写 <code>source_ref</code>、保留运行所需结构与外置受控资料分别说明，关键 <code>apor</code> 未解决时必须停止，不能靠删除未决项得到“干净”制品。普通接收者只能验证发布字节及适用陈述；没有受控伴随资料时，不能声称已经验证原始表达到发布结构的忠实转换。页面现有九个章节、七张边界表、三十四行表体、一条发布流程、四个继续阅读入口和 3,259 个可见字符。
...
本轮只重构公开 ADR、相邻入口、导航、架构索引、站点发现摘要、质量契约和审计记录。END-CORE、END-FMT、END-P1、字段、登记、场景、向量、现行形成制品和身份规则均未改变，也没有创建发布 Profile、字段映射、保留或删除清单、伴随资料格式、裁剪实现、公开 <code>strip</code> 动作、稳定 ABI、别名、重定向、双写或兼容垫片。

## 开放问题与 Profile 类型边界复核 · 2026-07-16

ADR-0036 已经确定形成版、发布版、新身份和验证范围，开放问题页与三个入口却仍把“来源保留与裁剪发布”整体列为正在研究，并把 END-P1 称为“实验性内容表示”或“实验性形成格式”。这些写法混合了已经确定的责任边界与仍待定义的物理表示，也把 Profile 误写成容器格式。开发者因而难以判断应继续研究发布 Profile，还是重新讨论已经由 END-CORE、ID-CORE 与 ADR 固定的对象关系。

当前页面直接回答“现在还缺什么证据”：END-P1 是含来源的形成与评审 Profile，END-FMT 0.1.0-draft 才是实验性物理容器；ADR-0036 与 <code>END-PUB-001</code> 已经固定形成版和发布版必须取得不同身份，待定项是删除、重写、保留、外置、来源闭包、伴随关系、披露政策与失败原子性怎样进入发布 Profile。关于页、架构入口和规范入口同步改正状态与类型。开放问题页仍保持五个章节、三张表、十三行表体和四个渐进披露分组，继续阅读入口由三个增至四个，可见正文由 2,898 个字符增至 3,061 个；新增文字只承担边界纠错和 ADR 路由。
...
本轮只调整公开研究路由、三个状态入口、站点发现摘要、质量契约和审计记录。END-CORE、END-FMT、END-P1、END-PUB-001、ID-CORE、ADR、登记、向量、研究提案、名称和读音结论均未改变，也没有创建发布 Profile、Synem 或 Iknem 编码、运行接口、组件或实现证据。
...
本轮只调整公开语言、无障碍替代文本、设计系统说明、两个非规范研究提案、质量契约与审计记录。Endem、END-CORE、END-P1、六个语义面、形成与发布关系、身份、授权、满足结果、名称和读音状态均未改变，也没有创建组件、格式或实现证据。
...
首页卡片继续保留六项语义职责、四类对象、三项信任责任和六条阅读路径，但摘要统一改成有主语的“输入—动作—结果—停止条件”表达。字段名只在直白职责之后出现；`rhem / semion / skena / telis / krin / apor` 不再承担首次解释。首页卡片目标也形成显式契约：六项职责进入 Endem 对应章节，四类对象进入各自规范，三项责任分别进入 Endem 应用、Theor 和 Drasor，继续阅读区进入应用、规范、架构、组件、项目进展和指南。
