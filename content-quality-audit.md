...
规范模块的引言以大平行四边形表示条款、编码和版本沿确定方向映射，以叠加的小平行四边形承载命题标签，章节标题前再用同方向的短标延续模块身份。此前把黑色命题面归入大形，使引言中的两层轮廓都采用左上贴齐，主次图形失去方向对比。

当前共享规则让彩色主形采用左上贴齐、左下收进的前向轮廓；黑色命题面和章节短标采用左下贴齐、左上收进的反向轮廓。大形与小形形成稳定的两种朝向，规范目录页和全部规范专题页通过同一组模块变量继承，不为外部协议适配页面建立特例。形状仍不裁切标题、徽标、链接或正文，也不改变规范含义、成熟度和实现状态。
