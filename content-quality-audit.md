...
规范模块的引言以大平行四边形表示条款、编码和版本沿确定方向映射，以小平行四边形承载命题标签。此前两层轮廓都让左下角贴齐、左上角收进，面积不同但方向相同，叠加后无法形成明确的主次关系。

当前共享规则把大轮廓改为左上贴齐、左下收进，小标签继续左下贴齐、左上收进。两层采用相反朝向，规范目录页和全部规范专题页通过同一个模块变量继承，不为外部协议适配页面建立特例。形状仍只位于引言装饰层，不裁切标题、徽标、链接或正文，也不改变规范含义、成熟度和实现状态。
