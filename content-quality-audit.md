...
| P0-W4 语言与构建 | C/Rust 双原型、6 个规范向量、144 个差分变异、Sanitizer、10,000 次 libFuzzer、重复构建和 `results.json` | ADR-0012 已为 Poiet 与生产读取核心选择 Rust 1.97.0；C 只作非生产差分预言机 | 仍需 Rust 覆盖引导目标和跨平台复现 |
| P1-W1 来源与安全核心 | 2 个 END-SRCM 来源样例、本地 Rust Poiet/Elenk/Theor/CLI、13 个 END-P1 字节向量与 3,372 个有界变异样本 | 两条读取路径对结构、Profile、语义错误给出相同条款、层次和位置；Poiet 在编码前执行图与文本预算 | 远端仓库、公开 CI、覆盖引导模糊、故障注入和跨平台二进制复现 |
