...
| P0-W3 规范向量 | 6 个语义 JSON 向量、6 个 END-P0 结构字节、11 个 END-P1 字节 | END-P1 覆盖两个接受、5 类语义拒绝、非最短 CBOR、图边、深度和字符串上限 | 仍缺截断、目录乱序与累计分配边界向量 |
...
| P1-W1 来源与安全核心 | 2 个 END-SRCM 来源样例、本地 Rust Poiet/Elenk/Theor/CLI、11 个 END-P1 字节向量与 3,372 个有界变异样本 | 两条读取路径独立拒绝编码、图边、深度和字符串超限；Poiet 在构造 CBOR 前执行图与文本预算 | 远端仓库、公开 CI、覆盖引导模糊、故障注入和跨平台二进制复现 |
