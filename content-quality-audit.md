...
| MCP 2025-11-25 稳定版与 2026-07-28 发布候选 | 当前稳定版仍是 2025-11-25；候选版改为无状态核心、扩展框架并强化授权，且明确包含破坏性变化 | 版本化外缘适配、最小能力、受众校验、拒绝和观察记录 | 候选版在正式发布与重验前不得进入基线；服务器说明、工具 schema、OAuth 身份和远端结果都不能直接成为本地授权或验收事实 | 2026-07-28 正式版发布，或授权、传输、工具结构变化 |
...
| 工作包 | 已形成证据 | 当前结论 | 剩余缺口 |
| --- | --- | --- | --- |
| P0-W1 权威规范 | `spec/endem-core.md`、`spec/endem-format.md`、`spec/endem-source-manifest.md`、43 个唯一条款 ID、END-P0/END-P1 与 `spec/registry.json` | 语义、结构、首个设计 Profile 和来源清单分层登记；实现和稳定 ABI 状态独立 | 代码阶段前继续用真实自然语言案例与反例修订；独立实现验证等待用户开启代码阶段 |
...
| P0-W3 场景与规范向量 | 8 个非规范自然语言设计场景、6 个语义 JSON 向量、6 个 END-P0 结构字节、13 个 END-P1 字节 | 场景覆盖达到、保持、否定、指称歧义、观察不足、求值故障、授权不足和多根拆分；字节向量覆盖两个接受及结构、语义和资源拒绝 | 场景不是可执行证据；仍缺时间区间、否定载荷、统计判据、累计分配、偏移算术和组合引用向量 |
