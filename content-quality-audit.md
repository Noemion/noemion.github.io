...
Noemion 已经接受以 Endem 为核心制品的工程词汇、单一应用拓扑和 END-FMT 实验性编码。首个本地 Rust 候选已实现四个动作并通过权威向量、独立读取、有界变异和对象字节复现，但实现仓库、安装包、稳定 ABI、跨平台结果和正式版本尚未公开。页面与本地测试存在不等于软件已经适合生产，也不证明性能、研究结论、知识产权或标准化状态。
...
本轮逐一复核 `sitemap.md` 登记的 43 条正式 HTML 路由，包括 30 个 HTML 正文源和 13 个由 Markdown 生成的页面。复核重点不是统一文风，而是让每种页面先完成自己的读者任务：门户给出项目定义，目录给出选择依据，专题给出结论与边界，应用给出状态与输入输出，手册给出连续操作逻辑。
...
| 开发（4） | `/development/index.html`、`/development/current-stage.html`、`/development/implementation-roadmap.html`、`/development/testing.html` | 阶段名称、验证条件和停止条件全部改为用户可读表达；当前已完成实现 Profile，正在建立核心工作区。 |
| 资源与支持（3） | `/downloads/index.html`、`/faq/index.html`、`/news/index.html` | 下载页直说为什么不能发布；FAQ 拆开 Synem、Dromen、Tekmor 定义；项目动态只陈述可核对进展。 |

本轮同时加入自动回归规则：现行页面不得出现旧扩展名、旧来源名、旧口号或未解释的内部英文职责名；普通段落中的单句不得超过 70 个汉字。思想基础页还必须保留经核对的作者、译者、书名、命题编号、短引文和“不直接决定软件规范”的采用边界。
...
| --- | --- | --- | --- | --- |
| ELF 对象格式与 GNU Binutils 2.46 | ELF 区分链接视图与装载视图；GNU `readelf` 可不经 BFD 读取对象；BFD 的通用转换可能丢失格式特有信息 | 结构与装载分离、显式引用、格式专用的独立第二读取路径 | 机器地址、指令、弱符号默认选择、BFD 通用内部表示 | GNU 主版本、ELF 格式或 Endem 物理编码变化 |
| MCP 2025-11-25 | 工具注解不是可信事实；HTTP 授权要求令牌绑定目标资源，禁止把入站令牌转交下游 | 版本化外缘适配、最小能力、受众校验、拒绝和观察记录 | 远端声明直接授予本地权限，或结果直接成为验收事实 | MCP 规范日期、授权模型、传输或工具结构变化 |
| A2A 1.0，文档快照 v1.0.1 | Task、Message 和 Artifact 服务跨系统交换；协议协商忽略补丁号 | 交换任务状态、消息和候选产物 | 外部 Task/Artifact 成为 Endem 身份、生命周期或最终决定 | A2A 主次版本、任务状态机或安全对象变化 |
| OpenTelemetry 生成式 AI 语义约定 | 字段正迁到独立仓库且仍在演进；输入、输出、工具参数和结果可能含敏感信息 | 带版本、默认脱敏、可替换的运行观测导出器 | 外部字段进入 Endem 编码、Tekmor 身份或验收规则 | 独立仓库首次稳定发布、字段稳定级别或隐私建议变化 |
...
| 工作包 | 已形成证据 | 当前结论 | 剩余缺口 |
| --- | --- | --- | --- |
| P0-W1 权威规范 | `spec/endem-core.md`、`spec/endem-format.md`、36 个唯一条款 ID、END-P0/END-P1 与 `spec/registry.json` | 语义、结构实验和首个封闭载荷 Profile 分层登记；实现和稳定 ABI 状态独立 | 继续用真实语料与两个独立实现反驳和修订 |
| P0-W2 威胁与限制 | `spec/endem-threat-model.md`、`spec/endem-errors.md`、`spec/profiles/end-p0.json` | 已建立 10 类威胁、8 项有限上限和结构错误目录 | 仍需边界规模、最坏复杂度和目标平台实验验证数值 |
| P0-W3 规范向量 | 6 个语义 JSON 向量、6 个 END-P0 结构字节、4 个 END-P1 完整载荷字节 | 已执行语义、结构与完整载荷三层结果；END-P1 覆盖接受、未知字段、悬空引用和来源范围越界 | 仍缺截断、乱序、非最短 CBOR 与资源边界向量 |
| P0-W4 语言与构建 | C/Rust 双原型、6 个规范向量、144 个差分变异、Sanitizer、10,000 次 libFuzzer、重复构建和 `results.json` | ADR-0012 已为 Poiet 与生产读取核心选择 Rust 1.97.0；C 只作非生产差分预言机 | 仍需建立生产工作区、Rust fuzz target、完整载荷和跨平台复现 |

`tests/spec_contract_test.py` 检查规范版本、条款唯一性、成熟度、威胁映射和向量登记；`tests/semantic_vector_test.py` 执行 JSON 语义外壳；`tests/wire_vector_test.py` 读取 END-P0 结构字节；`tests/p1_payload_test.py` 把同一最小语义模型确定性编码为 END-P1，再从完整字节解码字段与引用。它们证明当前规范资料与这些已执行案例一致，不证明未来生产实现符合规范。
