...
规范模块的引言以大平行四边形表示条款、编码和版本沿确定方向映射，黑色命题面在同一首屏承担主命题标签，章节标题前的短标再延续模块身份。此前按叠放层级把黑色命题面归为小形，导致它与真正的小型章节短标采用同一朝向。

当前共享规则按可见面积和页面职责分类：彩色主形与黑色命题面都采用左上贴齐、左下收进的前向轮廓；只有章节标题前的短标采用左下贴齐、左上收进的反向轮廓。规范目录页和全部规范专题页通过同一组模块变量继承，不为外部协议适配页面建立特例。形状仍不裁切标题、徽标、链接或正文，也不改变规范含义、成熟度和实现状态。
