...
| 工作包 | 已形成证据 | 当前结论 | 剩余缺口 |
| --- | --- | --- | --- |
| P0-W1 权威规范 | `spec/endem-core.md`、22 个唯一条款 ID、`spec/registry.json` | 第一份版本化条款骨架已经形成；接受的语义边界与实现状态分开登记 | 继续用真实语料反驳和修订；线格式另立规范 |
| P0-W2 威胁与限制 | `spec/endem-threat-model.md`、10 类威胁、8 项 Profile 必需上限 | 已建立攻击路径、保护目标、条款映射和停止边界 | 具体数值仍需对象规模、最坏复杂度和目标平台实验 |
| P0-W3 规范向量 | `vectors/vector.schema.json`、1 个接受向量、4 个拒绝向量 | 已能机械检查条款引用、拒绝类别和位置；向量明确不是 `.endem` 字节 | 仍缺规范字节、字段注释、资源边界和更多未知状态向量 |
| P0-W4 语言与构建 | 仅有路线图中的比较维度 | 尚未开始，不能宣称已选择实现语言 | 等物理格式草案和字节向量出现后再做同向量实验 |

`tests/spec_contract_test.py` 检查规范版本、条款唯一性、成熟度、威胁映射、向量登记和拒绝诊断。它证明仓库追溯关系自洽，不证明未来实现符合规范。
