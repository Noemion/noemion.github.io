...
本轮逐一复核 `sitemap.md` 登记的 42 条正式 HTML 路由，包括 29 个 HTML 正文源和 13 个由 Markdown 生成的页面。复核重点不是统一文风，而是让每种页面先完成自己的读者任务：门户给出项目定义，目录给出选择依据，专题给出结论与边界，应用给出状态与输入输出，手册给出连续操作逻辑。
...
| 工作包 | 已形成证据 | 当前结论 | 剩余缺口 |
| --- | --- | --- | --- |
| P0-W1 权威规范 | `spec/endem-core.md`、`spec/endem-format.md`、34 个唯一条款 ID、`spec/registry.json` | 语义与实验性容器拥有分离的权威条款；实现和稳定 ABI 状态独立登记 | 继续用真实语料与两个独立实现反驳和修订 |
...
| P0-W3 规范向量 | 6 个语义向量、6 个十六进制字节向量、两条独立测试入口 | 已执行 1 个语义接受与 5 个确定语义拒绝，并解码 1 个结构接受与 5 个确定结构拒绝；来源范围按 Unicode 标量实际核对 | 仍缺完整字段载荷、截断、乱序、非最短 CBOR 与资源边界向量 |
...
`tests/spec_contract_test.py` 检查规范版本、条款唯一性、成熟度、威胁映射和向量登记；`tests/semantic_vector_test.py` 实际执行六语义面、引用、根、目标方向、判断契约、未决项和来源范围检查；`tests/wire_vector_test.py` 独立读取头部、目录、范围与受限 CBOR，并比较稳定拒绝结果。它们证明当前规范资料与这些已执行案例一致，不证明未来生产实现符合规范。
