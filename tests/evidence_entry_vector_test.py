...
VECTOR_PATH = ROOT / "vectors" / "evidence-entry" / "cases.json"
CASE_ID = re.compile(r"^EVIDENCE-(?:VALID|REJECT)-[A-Z0-9-]+-[0-9]{3}$")
CLAUSES = {
    "EVIDENCE-SCP-001", "EVIDENCE-PRV-001", "EVIDENCE-OBS-001", "EVIDENCE-CLS-001",
    "EVIDENCE-INT-001", "EVIDENCE-VAL-001", "EVIDENCE-COV-001", "EVIDENCE-DEC-001",
    "EVIDENCE-PRI-001",
}
...
def proposal_violation(case):
    ...
    if observation is not None and (
        ...
        or observation.get("observation_aligned") is not True
        or observation.get("inference_as_observation") is not False
    ):
        return "EVIDENCE-OBS-001"
    if (
    ...
    if (
        ...
        return "EVIDENCE-VAL-001"
    coverage = proposal.get("coverage")
    if (
        not isinstance(coverage, dict)
        or not coverage.get("satisfaction_criteria")
