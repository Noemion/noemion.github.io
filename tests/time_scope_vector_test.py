...
VECTOR_PATH = ROOT / "vectors" / "time-scope" / "cases.json"
CASE_ID = re.compile(r"^TIME-(?:VALID|REJECT)-[A-Z0-9-]+-[0-9]{3}$")
...
def main():
    ...
    try:
        ...
        print(f"cannot read maintain vectors: {exc}")
        return 1

    if document.get("vector_format") != "end-core.time-scope-vector.v1":
        errors.append("maintain vectors must use end-core.time-scope-vector.v1")
    if document.get("spec") != {"id": "END-CORE", "version": "0.1.0-draft"}:
        errors.append("maintain vectors must pin END-CORE 0.1.0-draft")
    description = document.get("description", "")
    if "not a clock, monitor, bounded runner, or evaluator implementation" not in description:
        errors.append("maintain vectors must state their non-implementation boundary")

    cases = document.get("cases")
    if not isinstance(cases, list) or len(cases) != 12:
        errors.append("maintain vectors require exactly 12 proposal cases")
        cases = []
    ...
    if counts != {"accept": 6, "reject": 6}:
        errors.append(f"maintain vectors require 6 accepts and 6 rejects, got {counts}")
    if rejected_clauses != CLAUSES:
        errors.append(
            f"maintain reject coverage must include {sorted(CLAUSES)}, got {sorted(rejected_clauses)}"
        )
    ...
    if errors:
        ...
        return 1
    print(
        "PASS: executed 12 maintain vectors "
