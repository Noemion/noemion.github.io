...
- 两个或更多通过所需生产 `lint` 的 Endem；
- 声明命名空间、冲突、缺失、循环和权限规则的组合策略；
- 以内容摘要锁定外部对象的依赖锁；
- 关闭高风险 `unresolved_meaning` 项时所需的授权决定。
...
| 维度 | compose 必须保持 |
| --- | --- |
| `meaning_projection` | 符号、指称、关系位置和作用域不能静默改写 |
| `situation` | 每个 Endem 继续只有一个根中性事态，组合与极性不被改写 |
| `goal_direction` | reach/maintain 与时间边界只能保持或收紧 |
| `satisfaction_criteria` | 组合不能让满足条件更容易或丢失决定权威 |
| `unresolved_meaning` | 每项都被授权决定、继续阻断 resolved 状态或导致失败；不得改写为 undetermined |
| 能力 | 多对象组合只能保持或收紧授权上限 |
...
可选依赖必须声明缺失时的可观察行为。若缺失会改写 `meaning_projection`、削弱 `situation`、简化 `satisfaction_criteria` 或扩大能力，它就不能标为可选。
