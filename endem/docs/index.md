...
人写下的目标先保留为来源表达。只有解释已经按规则确认，`ktise` 才把它写成初始 Endem；含义不清时必须停止并报告问题。`elenk` 从写入一侧检查实际字节，`theor` 再用独立代码读取同一文件。目标需要引用其他目标时，`pleko` 固定全部依赖。外部系统可以对精确内容签名；真正运行前，`drase` 还要重新核对环境、能力和预算，并把观察写入 Iknem。
