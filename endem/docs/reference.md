...
同一个入口也不代表所有工作可以由同一套代码完成。写入目标文件、独立读取实际字节和限制一次运行的能力，必须由三个能够分别失败、分别验证的实现边界负责。
...
`inspect` 可以由同一个命令入口调度，但未来实现必须单独构建。它只能回答“这些字节在精确规则和预算下怎样显示、哪里不同或为何停止”，不能回答“这些字节已经通过生产检查”。
