...
| 直接职责 | 现行设计名称 | 必须保持的边界 |
| --- | --- | --- |
| 按固定规则写入目标制品 | deterministic producer | 只接收已经确认的意义，不持有签名密钥、实时能力或最终决定权 |
| 用另一套代码读取实际字节 | independent inspector | 不复用写入侧解析器，不修改输入，也不把可读视图写成生产检查通过 |
| 为一次运行限制能力 | bounded runner | 每次重新核对目标、授权、环境和预算，不拥有制品写入器 |
