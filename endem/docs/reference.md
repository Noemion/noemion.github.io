...
summary: "八个入口、Endem/Synem/Dromen/Iknem、状态、失败和权威参考。"
...
| 子命令 | 主要输入 | 主要输出 | 权限边界 |
| --- | --- | --- | --- |
| `poie` | 受控来源、投影候选与具名决定 | nascent Endem、Poie Iknem | 唯一规范写入入口 |
...
| `sphra` | 原候选、请求和外部签名响应 | attested 制品、Sphra Iknem | 不修改被签载荷 |
...
`.endem` 扩展名已经采用；参数、选项、退出码、Synem/Iknem 扩展名和尚未发布的安装接口仍未冻结。本表只冻结职责，不应据此编写依赖具体参数的脚本。
...
- [Poiet、Theor 与 Drasor](../../components/index.html)
