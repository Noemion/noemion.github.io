...
| Dromen | Praxor 为一次 Praxe 会话封存的只读执行契约，不是文件或可恢复权限 |
...
| Tekmor 有效性 | `valid / invalid / revoked` | 验证者在精确政策、参考值、环境和截止点下形成的外部评估 |
| 证据覆盖度 | `sufficient / insufficient` | 一组有效 Tekmor 是否覆盖指定 krin 与观察责任；不按记录数量计算 |
...
- `evaluation-coverage`：已求值叶、未求值叶、停止原因和所用 Tekmor。
...
DIA-CAT 0.1 已登记跨对象诊断、END-P0 结构错误和首组 END-P1 语义错误；完整 CLI 诊断 ABI 尚未冻结。未来实现还必须稳定区分以下更高层失败：
...
- [ADR-0022 Tekmor 证据范围与评估边界](../../architecture/adr-0022-tekmor-evidence-and-appraisal.html)
