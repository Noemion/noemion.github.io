...
`endem` 是当前规划的公开命令命名空间，但当前没有 CLI、安装包或可运行组件。这一页说明未来接口必须保持的职责，不是安装或命令参数说明，也不预先确定可执行文件、进程或发行包数量。

同一组动作名称不代表所有工作可以由同一套代码完成。写入目标文件、独立读取实际字节和限制一次运行的能力，必须由三个能够分别失败、分别验证的实现边界负责。

## 按工作查动作

| 要做的工作 | 动作 | 读取什么 | 产出与停止边界 |
| --- | --- | --- | --- |
| 从已确认解释形成目标制品 | `form` | 受控来源、投影候选与具名决定 | 形成来源保留的 formed Endem；只有 producer 可以承担规范字节生产责任；意义未确认时停止 |
| 从生产侧检查制品 | `lint` | 原始制品、检查层、限制和信任材料 | 返回分层结论；不替代独立读取，也不产生外部接受决定 |
| 固定多个目标的组合闭包 | `compose` | Endem、组合策略和依赖锁 | 形成完整组合闭包；引用未解析、存在冲突或权限扩大时停止 |
| 独立查看实际字节 | `inspect` | 当前为实际 Endem 字节、精确规范/Profile、视图和预算；其他对象等待物理格式 | 返回带范围的只读视图、差分和诊断；inspector 不修复、不写回，也不生成生产检查通过结论 |
| 建立一次受限会话 | `run` | 精确发布制品、外部陈述、验证政策、执行策略、后端与能力 | 形成 contract、会话结果和 evidence；runner 不拥有写入器，准入与接受由各自具名权威决定 |

`endem` 当前只表示公开命令命名空间，表中的五个名称是动作标识，不是已经发布的 CLI。当前只确定 Endem 使用 `.endem` 扩展名；最终工具数量、参数、选项、退出状态、安装接口以及 closure 和 evidence 的物理格式仍待确定，开发者不能据此编写自动化脚本。

## 三项安全责任为什么不能合并

| 直接职责 | 现行设计名称 | 必须保持的边界 |
| --- | --- | --- |
| 按固定规则写入目标制品 | producer | 只接收已经确认的意义，不持有签名密钥、实时能力或最终决定权 |
| 用另一套代码读取实际字节 | inspector | 不复用写入侧解析器，不修改输入，也不把可读视图写成生产检查通过 |
| 为一次运行限制能力 | runner | 每次重新核对目标、授权、环境和预算，不拥有制品写入器 |

`inspect` 可以由 `endem` 命令命名空间对外提供，但未来实现必须单独构建。它只能回答“这些字节在精确规则和预算下怎样显示、哪里不同或为何停止”，不能回答“这些字节已经通过生产检查”。

`run` 必须由受限运行边界承担。模型或其他后端只能提出候选和能力请求；实际观察进入有范围的证据记录，最终是否接受仍由具名决定者判断。
...
| closure | 两个或更多 Endem 的已解析组合闭包 | 不在运行时补依赖或扩大成员权限 | 只有抽象语义，物理格式待定 |
| contract | 一次 run 会话的只读执行契约 | 不是可转移文件，也不保存实时秘密或可恢复权限 | 只有抽象语义，序列化被明确禁止 |
| evidence | 有范围的观察、来源、方法、限制或决定依据 | 不自证有效，不替代满足判断与具名决定 | 只有抽象语义，物理格式待定 |
...
| 一项 evidence 在精确政策和截止点下是否可用 | `valid / invalid / revoked` | 证据覆盖已经充分或主张为真 |
| 一组有效 evidence 是否覆盖指定判断责任 | `sufficient / insufficient` | 按记录数量投票或自动形成最终决定 |
...
| --- | --- | --- | --- |
| 固定时间或经过时间 | UTC 半开区间或具名事件与单调时钟，以及 `strict/budgeted` 连续性 | 覆盖不足为 `undetermined`；时钟、同步或求值契约失败为 `fault` | [ADR-0016](../../architecture/adr-0016-time-evidence.html) |
...
| 证据与协议 `evidence/protocol` | evidence 范围、外部来源、协议版本、映射损失和取消语义 | `evidence.scope.unbound`、`adapter.error.provenance_lost` |
...
| 组合闭包和成员激活 | [closure 规范](../../specifications/endem-closure.html)、[绑定与组合](binding.html) |
| 独立读取、生产检查和签名边界 | [安全与独立检查](safety.html)、[producer、inspector 与 runner](../../components/index.html) |
| 会话建立、观察和受限运行 | [发布与运行](running.html)、[contract 会话契约](../../architecture/adr-0024-session-contract.html) |
| 证据范围、有效性和覆盖度 | [evidence 规范](../../specifications/evidence-entry.html)、[ADR-0022](../../architecture/adr-0022-evidence-and-appraisal.html) |
...
- 规范、ADR、登记和提案向量只说明当前设计的引用关系，不能证明组件已经实现。
- 当前只有 Endem 有实验性物理字节；closure 与 evidence 仍不能交给 inspector 作实际字节读取。
- 五个动作的参数、退出状态和安装接口尚未发布，诊断目录也未成为稳定 ABI。
- `form`、`lint`、`compose`、`inspect`、`run` 是已经通过词首、职责和关键字语料检查的现行设计标识；它们尚未发布为真实命令，发行时仍需复核当时的软件生态冲突。
- 项目尚未进入组件开发阶段，没有 producer、inspector、runner、求值器或可执行 `endem` 可供安装。

## 模型和外部工具放在哪里

数据、训练、评估和量化优先使用已有工具，不因为 Noemion 需要目标制品就另建一套模型平台。只有现有工具无法表达 Noemion 特有的数据来源、使用资格或设备限制，并且实验显示新增适配具有不可替代的收益时，项目才研究新的连接方式。

任何模型或外部工具都不能直接写入 Endem、授予运行权限或作出最终验收决定。
