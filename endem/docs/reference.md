...
计划中的 `endem` 是唯一公开命令入口，但当前没有 CLI、安装包或可运行组件。这一页说明未来接口必须保持的职责，不是安装或命令参数说明。
...
`endem` 计划作为唯一公开命令入口，表中的五个名称是动作标识，不是已经发布的 CLI。当前只确定 Endem 使用 `.endem` 扩展名；参数、选项、退出状态、安装接口以及 closure 和 evidence 的物理格式仍待确定，开发者不能据此编写自动化脚本。
